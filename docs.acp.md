...
This document describes how the Mayros ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`mayros acp` exposes an ACP agent over stdio and forwards prompts to a running
Mayros Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
In Zed, open the Agent panel and select “Mayros ACP” to start a thread.

## Execution Model

- ACP client spawns `mayros acp` and speaks ACP messages over stdio.
...
`mayros acp` resolves the Gateway URL and auth from CLI flags or config:
