...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: MayrosProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MayrosProtocol.AnyCodable] = [
            "presence": .init([MayrosProtocol.AnyCodable(entry)]),
        ]
        let payload = MayrosProtocol.AnyCodable(payloadMap)
