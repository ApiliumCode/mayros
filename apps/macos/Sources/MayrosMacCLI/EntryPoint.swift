...
struct MayrosMacCLI {
    static func main() async {
        ...
        default:
            fputs("mayros-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      mayros-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      mayros-mac discover [--timeout <ms>] [--json] [--include-local]
      mayros-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      mayros-mac connect
      mayros-mac connect --url ws://127.0.0.1:18789 --json
      mayros-mac discover --timeout 3000 --json
      mayros-mac wizard --mode local
    """)
}
