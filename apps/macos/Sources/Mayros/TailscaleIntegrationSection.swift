...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.mayros/mayros.json. Prefer MAYROS_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.mayros/mayros.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.mayros/mayros.json. Restart the gateway to apply."
        }
