...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MAYROS_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/mayros")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("mayros-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("mayros-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/mayros/ (rolling or stdout), or nil if none exist.
