...
private let webChatSwiftLogger = Logger(subsystem: "ai.mayros", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: MayrosChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> MayrosChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(MayrosChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MayrosChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MayrosGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MayrosChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MayrosChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MayrosChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: MayrosChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MayrosChatView>) -> NSViewController
    {
