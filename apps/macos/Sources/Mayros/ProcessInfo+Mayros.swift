...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.mayros.mac`) even if the shipped
    ...
    {
        if environment["MAYROS_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "mayros.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "mayros.nixMode") { return true }
