...
enum DebugActions {
    private static let verboseDefaultsKey = "mayros.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "mayros.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = MayrosPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Mayros", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = MayrosPaths.configURL
        guard
