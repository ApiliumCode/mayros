...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.mayros", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MayrosSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MayrosSessionPreviewEntry,
    ...
    private static func cache(payload: MayrosSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MayrosChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MayrosKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MayrosKit.AnyCodable]) -> [MayrosChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MayrosChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MayrosChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MayrosChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MayrosChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MayrosChatMessage) -> String? {
