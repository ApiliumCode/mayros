...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: MayrosOAuthStore.AnthropicOAuthStatus = MayrosOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: MayrosOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = MayrosOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = MayrosOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try MayrosOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Mayros can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: MayrosOAuthStore.AnthropicOAuthStatus,
