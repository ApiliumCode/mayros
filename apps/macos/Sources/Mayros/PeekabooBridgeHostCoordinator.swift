...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.mayros", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: MayrosPeekabooBridgeServices?
    private static var mayrosSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Mayros", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = MayrosPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.mayrosSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.mayrosSocketPath, privacy: .public)")
    }
...
@MainActor
private final class MayrosPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.mayros.peekaboo")
