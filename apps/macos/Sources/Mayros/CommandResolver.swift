...
enum CommandResolver {
    private static let projectRootDefaultsKey = "mayros.gatewayProjectRootPath"
    private static let helperName = "mayros"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let mayrosEntry = root.appendingPathComponent("mayros.mjs").path
        if FileManager().isReadableFile(atPath: mayrosEntry) { return mayrosEntry }
        let binEntry = root.appendingPathComponent("bin/mayros.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/mayros")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let mayrosPaths = self.mayrosManagedPaths(home: home)
        if !mayrosPaths.isEmpty {
            extras.insert(contentsOf: mayrosPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + mayrosPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func mayrosManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".mayros"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func mayrosExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMayrosExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("mayros.mjs").path,
            root.appendingPathComponent("bin/mayros.js").path,
        ]
    ...
    static func hasAnyMayrosInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.mayrosExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let mayrosPath = self.projectMayrosExecutable(projectRoot: root) {
                return [mayrosPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "mayros", subcommand] + extraArgs
            }
            if let mayrosPath = self.mayrosExecutable(searchPaths: searchPaths) {
                return [mayrosPath, subcommand] + extraArgs
            }

            let missingEntry = """
            mayros entrypoint missing (looked for dist/index.js or mayros.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real mayros CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/mayros"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/mayros.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/mayros.mjs"
            node "$PRJ/mayros.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/mayros.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/mayros.js"
            node "$PRJ/bin/mayros.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent mayros"
          pnpm --silent mayros \(quotedArgs);
        else
          echo "mayros CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MayrosConfigFile.loadDict()
