...
enum MayrosLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = MayrosOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MayrosFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MayrosLogging.bootstrapIfNeeded()
        let label = MayrosLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MayrosOSLogHandler: LogHandler {
...
struct MayrosFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MayrosLogging.parseLabel(self.label)
