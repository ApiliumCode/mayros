...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MayrosProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.mayros", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MayrosKit.AnyCodable]()) {
                $0[$1.key] = MayrosKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MayrosProtocol.AnyCodable?) -> [String: MayrosProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MayrosProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MayrosKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MayrosProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("mayros.control.heartbeat")
    static let controlAgentEvent = Notification.Name("mayros.control.agent")
}
