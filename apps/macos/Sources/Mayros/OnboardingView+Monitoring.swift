...
extension OnboardingView {
    ...
    func refreshLocalGatewayProbe() async {
        ...
        await MainActor.run {
            ...
            let expectedTokens = ["node", "mayros", "tsx", "pnpm", "bun"]
    ...
    func refreshAnthropicOAuthStatus() {
        _ = MayrosOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        let previous = self.anthropicAuthDetectedStatus
        let status = MayrosOAuthStore.anthropicOAuthStatus()
    ...
    func verifyAnthropicOAuthIfNeeded(force: Bool = false) async {
        ...
        guard let refresh = MayrosOAuthStore.loadAnthropicOAuthRefreshToken(), !refresh.isEmpty else {
        ...
        do {
            ...
            try MayrosOAuthStore.saveAnthropicOAuth(updated)
            self.refreshAnthropicOAuthStatus()
