...
private final class TestChatTransport: @unchecked Sendable, MayrosChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MayrosChatHistoryPayload]
    private let sessionsResponses: [MayrosChatSessionsListResponse]

    private let stream: AsyncStream<MayrosChatTransportEvent>
    private let continuation: AsyncStream<MayrosChatTransportEvent>.Continuation

    init(
        historyResponses: [MayrosChatHistoryPayload],
        sessionsResponses: [MayrosChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<MayrosChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MayrosChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MayrosChatHistoryPayload(
        ...
        attachments _: [MayrosChatAttachmentPayload]) async throws -> MayrosChatSendResponse
    {
        ...
        return MayrosChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MayrosChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MayrosChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MayrosChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = MayrosChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = MayrosChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { MayrosChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = MayrosChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = MayrosChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = MayrosChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { MayrosChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
