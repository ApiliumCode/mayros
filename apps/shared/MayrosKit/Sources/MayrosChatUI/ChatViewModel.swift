...
private let chatUILogger = Logger(subsystem: "ai.mayros", category: "MayrosChatUI")

@MainActor
@Observable
public final class MayrosChatViewModel {
    public private(set) var messages: [MayrosChatMessage] = []
    ...
    public var attachments: [MayrosPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MayrosChatPendingToolCall] = []
    public private(set) var sessions: [MayrosChatSessionEntry] = []
    private let transport: any MayrosChatTransport
    ...
    private var pendingToolCallsById: [String: MayrosChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any MayrosChatTransport) {
    ...
    public var sessionChoices: [MayrosChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [MayrosChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MayrosPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MayrosChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MayrosChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: MayrosChatMessage) -> MayrosChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> MayrosChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return MayrosChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: MayrosChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [MayrosChatMessage],
        incoming: [MayrosChatMessage]) -> [MayrosChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [MayrosChatMessage]) -> [MayrosChatMessage] {
        var result: [MayrosChatMessage] = []
    ...
    private static func dedupeKey(for message: MayrosChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MayrosChatMessageContent] = [
            MayrosChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> MayrosChatAttachmentPayload in
            MayrosChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MayrosChatSessionEntry {
        MayrosChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: MayrosChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MayrosChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MayrosAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MayrosChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MayrosPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
