...
struct ChatMessageBubble: View {
    let message: MayrosChatMessage
    let style: MayrosChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? MayrosChatTheme.userText : MayrosChatTheme.assistantText
    ...
    private var inlineAttachments: [MayrosChatMessageContent] {
    ...
    private var toolCalls: [MayrosChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [MayrosChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? MayrosChatTheme.userBubble
        }
        if self.style == .onboarding {
            return MayrosChatTheme.onboardingAssistantBubble
        }
        return MayrosChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return MayrosChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: MayrosChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? MayrosChatTheme.userText : MayrosChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: MayrosChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MayrosChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(MayrosChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [MayrosChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: MayrosChatTheme.assistantText)
            }
        }
    }
}
