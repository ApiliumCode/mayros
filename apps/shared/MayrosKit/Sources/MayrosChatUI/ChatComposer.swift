...
@MainActor
struct MayrosChatComposer: View {
    @Bindable var viewModel: MayrosChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(MayrosChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(MayrosChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \MayrosPendingAttachment.id)
                { (att: MayrosPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            MayrosPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MayrosChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(MayrosChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(MayrosChatTheme.subtleCard)
        .clipShape(Capsule())
    }
