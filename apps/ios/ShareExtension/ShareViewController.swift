...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.mayros.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Mayros", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Mayros gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Mayros.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "MayrosShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Mayros is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Mayros Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("mayros-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
