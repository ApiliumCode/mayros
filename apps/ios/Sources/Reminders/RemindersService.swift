...
final class RemindersService: RemindersServicing {
    func list(params: MayrosRemindersListParams) async throws -> MayrosRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MayrosReminderPayload], Error>) in
        ...
        return MayrosRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: MayrosRemindersAddParams) async throws -> MayrosRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = MayrosReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return MayrosRemindersAddPayload(reminder: payload)
    }
