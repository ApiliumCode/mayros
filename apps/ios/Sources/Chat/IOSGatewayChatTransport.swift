...
struct IOSGatewayChatTransport: MayrosChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.mayros", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> MayrosChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(MayrosChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MayrosChatHistoryPayload {
        ...
        return try JSONDecoder().decode(MayrosChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [MayrosChatAttachmentPayload]) async throws -> MayrosChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [MayrosChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(MayrosChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(MayrosGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MayrosGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MayrosChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MayrosAgentEventPayload.self)
                        {
