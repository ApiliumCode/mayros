...
@MainActor
final class MayrosAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.mayros.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.mayros.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.mayros.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "mayros.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "mayros.watch.promptId"
    static let sessionKeyKey = "mayros.watch.sessionKey"
    static let actionPrimaryIDKey = "mayros.watch.action.primary.id"
    static let actionPrimaryLabelKey = "mayros.watch.action.primary.label"
    static let actionSecondaryIDKey = "mayros.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "mayros.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "mayros.watch.action.primary"
    static let actionSecondaryIdentifier = "mayros.watch.action.secondary"
    static let categoryPrefix = "mayros.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: MayrosWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> MayrosWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return MayrosWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Mayros" : title
        content.body = body
    ...
    private static func categoryActions(
        primaryAction: MayrosWatchAction,
        secondaryAction: MayrosWatchAction?) -> [UNNotificationAction]
    {
...
@main
struct MayrosApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(MayrosAppDelegate.self) private var appDelegate
...
extension MayrosApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Mayros: installing uncaught exception handler")
