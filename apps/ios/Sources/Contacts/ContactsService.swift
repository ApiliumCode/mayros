...
final class ContactsService: ContactsServicing {
    ...
    func search(params: MayrosContactsSearchParams) async throws -> MayrosContactsSearchPayload {
        ...
        return MayrosContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: MayrosContactsAddParams) async throws -> MayrosContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return MayrosContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return MayrosContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> MayrosContactPayload {
        MayrosContactPayload(
