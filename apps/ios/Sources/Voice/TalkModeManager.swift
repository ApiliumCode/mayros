...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__MAYROS_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<MayrosTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = MayrosTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> MayrosTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return MayrosTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> MayrosTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> MayrosTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return MayrosTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> MayrosTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = MayrosTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: MayrosTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: MayrosAgentEventPayload.self) else {
                continue
            }
