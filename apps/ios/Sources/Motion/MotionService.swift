...
final class MotionService: MotionServicing {
    func activities(params: MayrosMotionActivityParams) async throws -> MayrosMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MayrosMotionActivityEntry], Error>) in
        ...
        return MayrosMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: MayrosPedometerParams) async throws -> MayrosPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<MayrosPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = MayrosPedometerPayload(
