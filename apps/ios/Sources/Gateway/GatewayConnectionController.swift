...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [MayrosCapability.canvas.rawValue, MayrosCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(MayrosCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(MayrosCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = MayrosLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(MayrosCapability.location.rawValue) }

        caps.append(MayrosCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(MayrosCapability.watch.rawValue)
        }
        caps.append(MayrosCapability.photos.rawValue)
        caps.append(MayrosCapability.contacts.rawValue)
        caps.append(MayrosCapability.calendar.rawValue)
        caps.append(MayrosCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(MayrosCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            MayrosScreenCommand.record.rawValue,
            MayrosSystemCommand.notify.rawValue,
            MayrosChatCommand.push.rawValue,
        ...
        if caps.contains(MayrosCapability.camera.rawValue) {
            commands.append(MayrosCameraCommand.list.rawValue)
            commands.append(MayrosCameraCommand.snap.rawValue)
            commands.append(MayrosCameraCommand.clip.rawValue)
        }
        if caps.contains(MayrosCapability.location.rawValue) {
            commands.append(MayrosLocationCommand.get.rawValue)
        }
        if caps.contains(MayrosCapability.device.rawValue) {
            commands.append(MayrosDeviceCommand.status.rawValue)
            commands.append(MayrosDeviceCommand.info.rawValue)
        }
        if caps.contains(MayrosCapability.watch.rawValue) {
            commands.append(MayrosWatchCommand.status.rawValue)
            commands.append(MayrosWatchCommand.notify.rawValue)
        }
        if caps.contains(MayrosCapability.photos.rawValue) {
            commands.append(MayrosPhotosCommand.latest.rawValue)
        }
        if caps.contains(MayrosCapability.contacts.rawValue) {
            commands.append(MayrosContactsCommand.search.rawValue)
            commands.append(MayrosContactsCommand.add.rawValue)
        }
        if caps.contains(MayrosCapability.calendar.rawValue) {
            commands.append(MayrosCalendarCommand.events.rawValue)
            commands.append(MayrosCalendarCommand.add.rawValue)
        }
        if caps.contains(MayrosCapability.reminders.rawValue) {
            commands.append(MayrosRemindersCommand.list.rawValue)
            commands.append(MayrosRemindersCommand.add.rawValue)
        }
        if caps.contains(MayrosCapability.motion.rawValue) {
            commands.append(MayrosMotionCommand.activity.rawValue)
            commands.append(MayrosMotionCommand.pedometer.rawValue)
        }
