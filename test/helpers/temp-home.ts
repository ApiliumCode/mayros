...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    mayrosHome: process.env.MAYROS_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("MAYROS_HOME", snapshot.mayrosHome);
  restoreKey("MAYROS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MAYROS_HOME.
  delete process.env.MAYROS_HOME;
  process.env.MAYROS_STATE_DIR = path.join(base, ".mayros");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "mayros-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".mayros", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
