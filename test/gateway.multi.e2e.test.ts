...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `mayros-e2e-${name}-`));
  const configDir = path.join(homeDir, ".mayros");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          MAYROS_SKIP_BROWSER_CONTROL_SERVER: "1",
