...
import type { MayrosConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("mayros-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("mayros-bin-sherpa-");
      const modelDir = await makeTempDir("mayros-sherpa-model-");
      ...
      const cfg: MayrosConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("mayros-bin-whispercpp-");
      const modelDir = await makeTempDir("mayros-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("mayros-bin-gemini-");
      ...
      const cfg: MayrosConfig = { tools: { media: { image: {} } } };
