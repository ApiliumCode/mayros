...
const shouldBuild = (deps) => {
  if (deps.env.MAYROS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MAYROS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[mayros] ${message}\n`);
};

const runMayros = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["mayros.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
