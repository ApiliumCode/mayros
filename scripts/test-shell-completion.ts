 ...
 * `mayros update`. Use it to verify the completion installation flow
...
\`mayros update\`, \`mayros doctor\`, and \`mayros onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Mayros Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`mayros completion --install\` later to enable.`));
    return;
  }
