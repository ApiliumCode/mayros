...
# Build and bundle Mayros into a minimal .app we can open.
# Outputs to dist/Mayros.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Mayros.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Mayros"
BUNDLE_ID="${BUNDLE_ID:-ai.mayros.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/ApiliumCode/mayros/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Mayros/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MayrosBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MayrosGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Mayros"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Mayros"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Mayros"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Mayros" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Mayros/Resources/Mayros.icns" "$APP_ROOT/Contents/Resources/Mayros.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Mayros/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MayrosKit resources"
MAYROSKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MayrosKit_MayrosKit.bundle"
if [ -d "$MAYROSKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MayrosKit_MayrosKit.bundle"
  cp -R "$MAYROSKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MayrosKit_MayrosKit.bundle"
else
  echo "WARN: MayrosKit resource bundle not found at $MAYROSKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Mayros"
killall -q Mayros 2>/dev/null || true
