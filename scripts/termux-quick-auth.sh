...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/mayros/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Mayros Re-Auth" -c "After getting key, run: ssh $SERVER '~/mayros/scripts/mobile-reauth.sh'" --id mayros-auth
        ;;
    *)
