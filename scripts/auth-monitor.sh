...
#
# Suggested cron: */30 * * * * /home/admin/mayros/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Mayros notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., mayros-alerts)
...
STATE_FILE="$HOME/.mayros/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Mayros if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use mayros
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Mayros to $NOTIFY_PHONE..."
            mayros send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Mayros is down. Run: ssh l36 '~/mayros/scripts/mobile-reauth.sh'" "urgent"
    exit 1
