...
IMAGE_NAME="mayros-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="mayros-net-e2e-$$"
GW_NAME="mayros-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "MAYROS_GATEWAY_TOKEN=$TOKEN" \
	  -e "MAYROS_SKIP_CHANNELS=1" \
	  -e "MAYROS_SKIP_GMAIL_WATCHER=1" \
	  -e "MAYROS_SKIP_CRON=1" \
	  -e "MAYROS_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
