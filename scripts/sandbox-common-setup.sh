...
BASE_IMAGE="${BASE_IMAGE:-mayros-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-mayros-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=mayros.sandbox=1)
NOTE
