...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Mayros-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/ApiliumCode/mayros/releases/download/v${VERSION}/"}
