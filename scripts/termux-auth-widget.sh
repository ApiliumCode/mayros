...
# Check auth status
termux-toast "Checking Mayros auth..."

STATUS=$(ssh "$SERVER" '$HOME/mayros/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/mayros/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|MAYROS_EXPIRING|MAYROS_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/mayros/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|MAYROS_EXPIRED|MAYROS_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Mayros won't work until re-auth"
                ;;
        esac
        ;;
