...
INSTALL_URL="${MAYROS_INSTALL_URL:-https://mayros.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${MAYROS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MAYROS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="mayros"
PACKAGE_NAME="${MAYROS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MAYROS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MAYROS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MAYROS_INSTALL_LATEST_OUT:-}"
fi
