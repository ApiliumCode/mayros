...
export async function handleWhatsAppStart(host: MayrosApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: MayrosApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: MayrosApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: MayrosApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: MayrosApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: MayrosApp): string {
...
function resolveGatewayHttpAuthHeader(host: MayrosApp): string | null {
...
function buildGatewayHttpHeaders(host: MayrosApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: MayrosApp) {
...
export function handleNostrProfileToggleAdvanced(host: MayrosApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: MayrosApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: MayrosApp) {
  const state = host.nostrProfileFormState;
