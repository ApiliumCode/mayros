...
export function connectGateway(host: GatewayHost) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void loadAssistantIdentity(host as unknown as MayrosApp);
      void loadAgents(host as unknown as MayrosApp);
...
function handleGatewayEventUnsafe(host: GatewayHost, evt: GatewayEventFrame) {
  ...
  if (evt.event === "chat") {
    ...
    const state = handleChatEvent(host as unknown as MayrosApp, payload);
    if (state === "final" || state === "error" || state === "aborted") {
      ...
      if (runId && host.refreshSessionsAfterChat.has(runId)) {
        ...
        if (state === "final") {
          void loadSessions(host as unknown as MayrosApp, {
    ...
    if (state === "final") {
      void loadChatHistory(host as unknown as MayrosApp);
    }
    return;
  }
