...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/mayros/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/mayros");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/mayros/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__MAYROS_CONTROL_UI_BASE_PATH__ = "/mayros";
    const app = mountApp("/mayros/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/mayros");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/mayros/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("mayros.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
