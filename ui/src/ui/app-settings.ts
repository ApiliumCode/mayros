...
import type { MayrosApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as MayrosApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as MayrosApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as MayrosApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as MayrosApp);
    await loadConfig(host as unknown as MayrosApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as MayrosApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as MayrosApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as MayrosApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as MayrosApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as MayrosApp);
    await loadDevices(host as unknown as MayrosApp);
    await loadConfig(host as unknown as MayrosApp);
    await loadExecApprovals(host as unknown as MayrosApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as MayrosApp);
    await loadConfig(host as unknown as MayrosApp);
  }
  if (host.tab === "debug") {
  ...
  if (host.tab === "canvas") {
    await loadCanvasSurface(host as unknown as MayrosApp);
  }
  if (host.tab === "usage") {
...
export function inferBasePath() {
  ...
  const configured = window.__MAYROS_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as MayrosApp, false),
    loadPresence(host as unknown as MayrosApp),
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as MayrosApp, true),
    loadConfigSchema(host as unknown as MayrosApp),
    loadConfig(host as unknown as MayrosApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as MayrosApp, false),
    loadCronStatus(host as unknown as MayrosApp),
    loadCronJobs(host as unknown as MayrosApp),
  ]);
}
