...
Tlon is a decentralized messenger built on Urbit. Mayros connects to your Urbit ship and can
...
```bash
mayros plugins install @apilium/mayros-tlon
```

Local checkout (when running from a git repo):

```bash
mayros plugins install ./extensions/tlon
```
...
- Thread replies: if the inbound message is in a thread, Mayros replies in-thread.
