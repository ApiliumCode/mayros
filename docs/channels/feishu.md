...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Mayros to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
mayros plugins install @apilium/mayros-feishu
```

Local checkout (when running from a git repo):

```bash
mayros plugins install ./extensions/feishu
```
...
If you just installed Mayros, run the wizard:
...
2. Configuring app credentials in Mayros
...
1. You already ran `mayros channels add` for Feishu
2. The gateway is running (`mayros gateway status`)
...
```bash
mayros pairing approve feishu <CODE>
```
...
2. Run `mayros logs --follow` and look for `chat_id`
...
2. Run `mayros logs --follow` and look for `open_id`
...
| Command                  | Description                   |
| ------------------------ | ----------------------------- |
| `mayros gateway status`  | Show gateway status           |
| `mayros gateway install` | Install/start gateway service |
| `mayros gateway stop`    | Stop gateway service          |
| `mayros gateway restart` | Restart gateway service       |
| `mayros logs --follow`   | Tail gateway logs             |
...
4. Check logs: `mayros logs --follow`
...
5. Ensure the gateway is running: `mayros gateway status`
6. Check logs: `mayros logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "atlas-fan",
        workspace: "/home/user/atlas-fan",
        agentDir: "/home/user/.mayros/agents/atlas-fan/agent",
      },
      {
        id: "atlas-xi",
        workspace: "/home/user/atlas-xi",
        agentDir: "/home/user/.mayros/agents/atlas-xi/agent",
      },
    ],
  },
  bindings: [
