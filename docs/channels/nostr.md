...
read_when:
  - You want Mayros to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Mayros to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`mayros onboard`) and `mayros channels add` list optional channel plugins.
...
```bash
mayros plugins install @apilium/mayros-nostr
```

Use a local checkout (dev workflows):

```bash
mayros plugins install --link <path-to-mayros>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "mayros@example.com",
        "lud16": "mayros@example.com"
      }
    }
  }
}
