...
read_when:
  - Setting up Twitch chat integration for Mayros
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Mayros connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
mayros plugins install @apilium/mayros-twitch
```

Local checkout (when running from a git repo):

```bash
mayros plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `MAYROS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "mayros", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use MAYROS_TWITCH_ACCESS_TOKEN env var)
...
```bash
MAYROS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
