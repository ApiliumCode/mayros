...
```bash
mayros plugins install @apilium/mayros-nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
mayros plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Mayros" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `mayros pairing list nextcloud-talk`
  - `mayros pairing approve nextcloud-talk <CODE>`
