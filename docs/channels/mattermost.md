---
summary: "Mattermost bot setup and Mayros config"
read_when:
...
```bash
mayros plugins install @apilium/mayros-mattermost
```

Local checkout (when running from a git repo):

```bash
mayros plugins install ./extensions/mattermost
```
...
4. Configure Mayros and start the gateway.
...
- Approve via:
  - `mayros pairing list mattermost`
  - `mayros pairing approve mattermost <CODE>`
...
Use these target formats with `mayros message send` or cron/webhooks:
