...
Matrix is an open, decentralized messaging protocol. Mayros connects as a Matrix **user**
...
```bash
mayros plugins install @apilium/mayros-matrix
```

Local checkout (when running from a git repo):

```bash
mayros plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `mayros plugins install @apilium/mayros-matrix`
   - From a local checkout: `mayros plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Mayros calls the same
     login endpoint, stores the access token in `~/.mayros/credentials/matrix/credentials.json`,
...
- On first connection, Mayros requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Mayros logs a warning.
...
`~/.mayros/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `mayros pairing list matrix`
  - `mayros pairing approve matrix <CODE>`
...
- On startup, Mayros resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
