---
summary: "Symptom first troubleshooting hub for Mayros"
read_when:
  - Mayros is not working and you need the fastest path to a fix
...
- `mayros gateway status` → `Runtime: running` and `RPC probe: ok`.
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `mayros gateway status`
    - `RPC probe: ok`
  ...
  <Accordion title="Browser tool fails">
    ...
    - `mayros` profile starts or `chrome` relay has an attached tab.
