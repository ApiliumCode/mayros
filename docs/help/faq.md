---
summary: "Frequently asked questions about Mayros setup, configuration, and usage"
title: "FAQ"
---
...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up Mayros?](#whats-the-recommended-way-to-install-and-set-up-mayros)
  ...
  - [I can't access apilium.com/us/doc/mayros (SSL error). What now?](#i-cant-access-apiliumcomusdocmayros-ssl-error-what-now)
  - [What's the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
  - [How do I install the beta version, and what's the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or mayros not recognized](#windows-install-says-git-not-found-or-mayros-not-recognized)
  - [The docs didn't answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install Mayros on Linux?](#how-do-i-install-mayros-on-linux)
  - [How do I install Mayros on a VPS?](#how-do-i-install-mayros-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask Mayros to update itself?](#can-i-ask-mayros-to-update-itself)
  ...
  - [If I buy a Mac mini to run Mayros, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-mayros-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different Mayros instances?](#can-multiple-people-use-one-whatsapp-number-with-different-mayros-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run Mayros on a dedicated machine?](#how-important-is-it-to-run-mayros-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run Mayros in a VM and what are the requirements](#can-i-run-mayros-in-a-vm-and-what-are-the-requirements)
- [What is Mayros?](#what-is-mayros)
  - [What is Mayros, in one paragraph?](#what-is-mayros-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for Mayros](#what-are-the-top-five-everyday-use-cases-for-mayros)
  - [Can Mayros help with lead gen outreach ads and blogs for a SaaS](#can-mayros-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can Mayros run tasks on a schedule or continuously in the background?](#can-mayros-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with Mayros saved locally?](#is-all-data-used-with-mayros-saved-locally)
  - [Where does Mayros store its data?](#where-does-mayros-store-its-data)
  - [Where should AGENTS.md / MAYROS.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall Mayros?](#how-do-i-completely-uninstall-mayros)
...
- [Config basics](#config-basics)
  ...
  - [Can the Mayros browser run headless?](#can-the-mayros-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two Mayros instances talk to each other (local + VPS)?](#can-two-mayros-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does Mayros load environment variables?](#how-does-mayros-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of Mayros instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-mayros-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset Mayros but keep it installed?](#how-do-i-completely-reset-mayros-but-keep-it-installed)
  ...
  - [Why doesn't Mayros reply in a group?](#why-doesnt-mayros-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do Mayros, Sentinel, and Pixel use for models?](#what-do-mayros-sentinel-and-pixel-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `mayros gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-mayros-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `mayros gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-mayros-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run Mayros in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-mayros-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart Mayros?](#i-closed-my-terminal-on-windows-how-do-i-restart-mayros)
  ...
  - [ELI5: `mayros gateway restart` vs `mayros gateway`](#eli5-mayros-gateway-restart-vs-mayros-gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media and attachments](#media-and-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose Mayros to inbound DMs?](#is-it-safe-to-expose-mayros-to-inbound-dms)
...
7. **Gateway snapshot**
   ...
   ```bash
   mayros health --json
   mayros health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://mayros.apilium.com/install.sh | bash -s -- --install-method git
```

This installs Mayros **from a git checkout**, so the agent can read the code + docs and
...
[https://github.com/ApiliumCode/mayros/issues](https://github.com/ApiliumCode/mayros/issues)
[https://github.com/ApiliumCode/mayros/pulls](https://github.com/ApiliumCode/mayros/pulls)
...
- `mayros status`: quick snapshot of gateway/agent health + basic config.
- `mayros models status`: checks provider auth + model availability.
- `mayros doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `mayros status --all`, `mayros logs --follow`,
`mayros gateway status`, `mayros health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up Mayros

The repo recommends running from source and using the onboarding wizard:

```bash
curl -fsSL https://mayros.apilium.com/install.sh | bash
...
If you don't have a global install yet, run it via `pnpm mayros onboard`.
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `MAYROS_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `mayros config get gateway.auth.token` (or generate one: `mayros doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `mayros gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `mayros gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install Mayros on the new machine.
2. Copy `$MAYROS_STATE_DIR` (default: `~/.mayros`) from the old machine.
3. Copy your workspace (default: `~/.mayros/workspace`).
4. Run `mayros doctor` and restart the Gateway service.
...
under `~/.mayros/` (for example `~/.mayros/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-mayros-store-its-data),
...
[https://github.com/ApiliumCode/mayros/blob/main/CHANGELOG.md](https://github.com/ApiliumCode/mayros/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I can't access apilium.com/us/doc/mayros SSL error What now

Some Comcast/Xfinity connections incorrectly block `apilium.com/us/doc/mayros` via Xfinity
Advanced Security. Disable it or allowlist `apilium.com/us/doc/mayros`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsmayrosai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/ApiliumCode/mayros/tree/main/docs](https://github.com/ApiliumCode/mayros/tree/main/docs)
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://mayros.apilium.com/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://mayros.apilium.com/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://mayros.apilium.com/install.ps1](https://mayros.apilium.com/install.ps1)
...
```bash
curl -fsSL https://mayros.apilium.com/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://mayros.apilium.com/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://mayros.apilium.com/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://mayros.apilium.com/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or mayros not recognized
...
**2) mayros is not recognized after install**
...
### How do I install Mayros on Linux
...
### How do I install Mayros on a VPS
...
### Can I ask Mayros to update itself
...
`mayros onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run Mayros with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `mayros models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `mayros models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `mayros models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. Mayros no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so Mayros can keep replying while a provider is rate-limited.
...
Mayros supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.3-codex` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. Mayros fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `mayros.json`.

Steps:

1. Enable the plugin: `mayros plugins enable google-gemini-cli-auth`
...
Usually no. Mayros needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. Mayros runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run Mayros can I connect it to my MacBook Pro
...
- Use `mayros nodes status` / `mayros nodes list` to see it.
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but Mayros authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `mayros logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different Mayros instances
...
If you run Mayros via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the Mayros code install. Your state
(`~/.mayros`) and workspace (`~/.mayros/workspace`) stay untouched.
...
**Mayros-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run Mayros on a dedicated machine
...
Mayros is lightweight. For a basic Gateway + one chat channel:
...
### Can I run Mayros in a VM and what are the requirements
...
### What is Mayros in one paragraph

Mayros is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

Mayros is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for Mayros
...
### Can Mayros help with lead gen outreach ads and blogs for a SaaS
...
Mayros draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

Mayros is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use Mayros when you
...
Showcase: [https://apilium.com/us/products/mayros/showcase](https://apilium.com/us/products/mayros/showcase)

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.mayros/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.mayros/mayros.json`). Precedence is `<workspace>/skills` > `~/.mayros/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.mayros/mayros.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.mayros/skills` → bundled → `skills.load.extraDirs`. `skillshub` installs into `./skills` by default, which Mayros treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `MAYROS_SKIP_CRON` is not set.
...
Use **Skills Hub** (CLI) or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
Browse skills at [https://hub.apilium.com](https://hub.apilium.com).

Install the Skills Hub CLI (pick one package manager):
...
```bash
pnpm add -g skillshub
```

### Can Mayros run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.mayros.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-mayros-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Mayros can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.mayros/skills`) to allow Linux:
...
```bash
skillshub install <skill-slug>
skillshub update --all
```

Skills Hub installs into `./skills` under your current directory (or falls back to your configured Mayros workspace); Mayros treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.mayros/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [Skills Hub](/tools/skillshub).
...
- Persist `/home/node` with `MAYROS_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `MAYROS_DOCKER_APT_PACKAGES`.
...
Mayros memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

Mayros also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, Mayros auto-selects a provider when it
can resolve an API key (auth profiles, `models.providers.*.apiKey`, or env vars).
It prefers OpenAI if an OpenAI key resolves, otherwise Gemini if a Gemini key
resolves. If neither key is available, memory search stays disabled until you
configure it. If you have a local model path configured and present, Mayros
prefers `local`.
...
### Is all data used with Mayros saved locally

No - **Mayros's state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.mayros` + your workspace directory).
...
### Where does Mayros store its data

Everything lives under `$MAYROS_STATE_DIR` (default: `~/.mayros`):

| Path                                                          | Purpose                                                      |
| ------------------------------------------------------------- | ------------------------------------------------------------ |
| `$MAYROS_STATE_DIR/mayros.json`                               | Main config (JSON5)                                          |
| `$MAYROS_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use) |
| `$MAYROS_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys)                             |
| `$MAYROS_STATE_DIR/agents/<agentId>/agent/auth.json`          | Runtime auth cache (managed automatically)                   |
| `$MAYROS_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)      |
...
| `$MAYROS_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                 |

Legacy single-agent path: `~/.mayros/agent/*` (migrated by `mayros doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.mayros/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.mayros`.

- **Workspace (per agent)**: `AGENTS.md`, `MAYROS.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.mayros`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.mayros/skills`).

Default workspace is `~/.mayros/workspace`, configurable via:
...
Do **not** commit anything under `~/.mayros` (credentials, sessions, tokens).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall Mayros
...
`workspace` to the repo root. The Mayros repo is just source code; keep the
...
Mayros reads an optional **JSON5** config from `$MAYROS_CONFIG_PATH` (default: `~/.mayros/mayros.json`):

```
$MAYROS_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.mayros/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `MAYROS_GATEWAY_TOKEN`).
...
Mayros enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `mayros doctor --generate-gateway-token`.
...
key. **Recommended:** run `mayros configure --section web` to store it in
...
- Daemons read env vars from `~/.mayros/.env` (or the service environment).
...
### Can the Mayros browser run headless
...
- Gateway is running: `mayros gateway status`
...
### Can two Mayros instances talk to each other local VPS
...
`mayros agent --message ... --deliver`, targeting a chat where the other bot
...
- Restore from backup (git or a copied `~/.mayros/mayros.json`).
- If you have no backup, re-run `mayros doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `mayros config set` for small changes.
- Use `mayros configure` for interactive edits.
...
### How does Mayros load environment variables

Mayros reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.mayros/.env` (aka `$MAYROS_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.mayros/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`MAYROS_LOAD_SHELL_ENV=1`, `MAYROS_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`mayros models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means Mayros won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.mayros/.env`:
...
### Is there a way to make a team of Mayros instances one CEO and many agents
...
### How do I completely reset Mayros but keep it installed
...
- If you used profiles (`--profile` / `MAYROS_PROFILE`), reset each state dir (defaults are `~/.mayros-<profile>`).
- Dev reset: `mayros gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), Mayros skips the heartbeat run to save API calls.
...
No. Mayros runs on **your own account**, so if you're in the group, Mayros can see it.
...
### Why doesn't Mayros reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.mayros/agents/<agentId>/sessions/`.
...
- Use `mayros doctor` to spot stray workspaces and profile mismatches.
...
Mayros's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Mayros currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `mayros models set ...` (updates just model config)
- `mayros configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.mayros/mayros.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `mayros doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do Mayros, Sentinel, and Pixel use for models

- **Mayros + Sentinel:** Anthropic Opus (`anthropic/claude-opus-4-6`) - see [Anthropic](/providers/anthropic).
- **Pixel:** MiniMax M2.1 (`minimax/MiniMax-M2.1`) - see [MiniMax](/providers/minimax).
...
Yes. Mayros ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
- Run `mayros agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so Mayros can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  ...
  - Legacy: `~/.mayros/agent/*` (migrated by `mayros doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.mayros/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `mayros models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `mayros models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `mayros models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.mayros/.env` on the **gateway host**.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Mayros will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: Mayros now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
Mayros uses provider-prefixed IDs like:
...
Mayros may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `mayros models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
### Why does mayros gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `mayros gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does mayros gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `MAYROS_STATE_DIR` mismatch).
...
Mayros enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `mayros gateway --port <port>`.

### How do I run Mayros in remote mode client connects to a Gateway elsewhere
...
- `mayros gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `mayros.control.settings.v1`.

Fix:

- Fastest: `mayros dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `mayros doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
- Set `gateway.auth.token` (or `MAYROS_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token.
- Still stuck? Run `mayros status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- Use `mayros --profile <name> …` per instance (auto-creates `~/.mayros-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `mayros --profile <name> gateway install`.

Profiles also suffix service names (`ai.mayros.<profile>`; legacy `com.mayros.*`, `mayros-gateway-<profile>.service`, `Mayros Gateway (<profile>)`).
...
- macOS: `$MAYROS_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.mayros/logs/...`; profiles use `~/.mayros-<profile>/logs/...`)
...
If you run the gateway manually, `mayros gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart Mayros
...
1. Is the Gateway running? `mayros gateway status`
2. Is the Gateway healthy? `mayros status`
3. Does the UI have the right token? `mayros dashboard`
...
- `mayros gateway restart`: restarts the **background service** (launchd/systemd).
- `mayros gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `mayros gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Mayros assistant setup](/start/mayros) and [Agent send](/tools/agent-send).
...
### Is it safe to expose Mayros to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `mayros pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `mayros pairing list <channel>` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `mayros doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Mayros only replies to chats it receives or to explicit sends you trigger.
...
Mayros blocks **cross-provider** messaging by default. If a tool call is bound
