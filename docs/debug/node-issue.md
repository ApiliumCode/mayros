...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Mayros
...
Running Mayros via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `mayros status` and `gateway:watch` worked.
...
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node mayros.mjs status` works on Node 25.
