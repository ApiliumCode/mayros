---
summary: "Mayros CLI reference for `mayros` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.mayros-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.mayros-<name>`.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `mayros update` (source installs only).
...
Mayros uses a mayros palette for CLI output.
...
Palette source of truth: `src/terminal/palette.ts` (aka “mayros seam”).
...
Note: plugins can add additional top-level commands (for example `mayros voicecall`).

## Security

- `mayros security audit` — audit config + local state for common security foot-guns.
- `mayros security audit --deep` — best-effort live Gateway probe.
- `mayros security audit --fix` — tighten safe defaults and chmod state/config.

## Plugins

Manage extensions and their config:

- `mayros plugins list` — discover plugins (use `--json` for machine output).
- `mayros plugins info <id>` — show details for a plugin.
- `mayros plugins install <path|.tgz|npm-spec>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `mayros plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `mayros plugins doctor` — report plugin load errors.
...
- `mayros memory status` — show index stats.
- `mayros memory index` — reindex memory files.
- `mayros memory search "<query>"` — semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/.mayros/workspace`).
...
Non-interactive config helpers (get/set/unset). Running `mayros config` with no
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `mayros health` or `mayros status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `mayros doctor`).
...
Mayros can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `mayros status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Mayros services are treated as first-class and aren't flagged as "extra".
...
`mayros models` is an alias for `models status`.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`mayros browser`](/cli/browser) and the [Browser tool](/tools/browser).
