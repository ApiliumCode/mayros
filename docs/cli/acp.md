...
Run the ACP (Agent Client Protocol) bridge that talks to a Mayros Gateway.
...
# Override the server command (default: mayros)
...
it to drive a Mayros Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `mayros acp` over stdio.
...
- Prefer `--token-file`/`--password-file` or environment variables (`MAYROS_GATEWAY_TOKEN`, `MAYROS_GATEWAY_PASSWORD`).

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `mayros`).
