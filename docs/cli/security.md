---
summary: "CLI reference for `mayros security` (audit and fix common security footguns)"
read_when:
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `mayros.browserConfigEpoch`) and recommends `mayros sandbox recreate --browser --all`.
