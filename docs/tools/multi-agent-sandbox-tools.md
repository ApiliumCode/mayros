...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `mayros sandbox explain`.
...
Legacy `agent.*` configs are migrated by `mayros doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=mayros-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${MAYROS_STATE_DIR:-$HOME/.mayros}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
