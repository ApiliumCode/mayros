---
summary: "Chrome extension: let Mayros drive your existing Chrome tab"
read_when:
...
The Mayros Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate mayros-managed Chrome profile.
...
Mayros then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Mayros release (npm package) as static files. There is no separate “build” step.

After upgrading Mayros:

- Re-run `mayros browser extension install` to refresh the installed files under your Mayros state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Mayros ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `MAYROS_GATEWAY_TOKEN`)
...
- Open the tab you want Mayros to control.
...
- `ON`: attached; Mayros can drive that tab.
...
Debugging: `mayros sandbox explain`
...
`mayros browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `mayros browser extension install` first to copy the extension to a stable location under your Mayros state directory.
...
- **This is not isolated** like the dedicated mayros-managed profile.
