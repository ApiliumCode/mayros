...
- **gateway host** → `mayros` process on the gateway machine
...
`~/.mayros/exec-approvals.json` directly.

CLI: `mayros approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
