...
macOS can also run in **node mode**: the menubar app connects to the Gateway’s WS server and exposes its local canvas/camera commands as a node (so `mayros nodes …` works against this Mac).
...
- `node.pair.*` (CLI: `mayros nodes pending/approve/reject`) is a separate gateway-owned
...
- **Approvals**: enforced on the node host via `~/.mayros/exec-approvals.json`.
...
- The token is `gateway.auth.token` from the gateway config (`~/.mayros/mayros.json` on the gateway host).
- `mayros node run` reads `MAYROS_GATEWAY_TOKEN` for auth.
...
- `--display-name` on `mayros node run` / `mayros node install` (persists in `~/.mayros/node.json` on the node).
...
Approvals live on the node host at `~/.mayros/exec-approvals.json`.
...
mayros nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
...
- On headless node host, `system.run` is gated by exec approvals (`~/.mayros/exec-approvals.json`).
...
Mayros can run a **headless node host** (no UI) that connects to the Gateway
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.mayros/node.json`.
- Exec approvals are enforced locally via `~/.mayros/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host prefers the companion app exec host when reachable and falls
  back to local execution if the app is unavailable. Set `MAYROS_NODE_EXEC_HOST=app` to require
  the app, or `MAYROS_NODE_EXEC_FALLBACK=0` to disable fallback.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `mayros nodes …` works against this Mac).
