...
Mayros supports **camera capture** for agent workflows:
...
```bash
mayros nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
...
Use the main `mayros` CLI to invoke camera commands on the macOS node.

Examples:

```bash
mayros nodes camera list --node <id>            # list camera ids
mayros nodes camera snap --node <id>            # prints MEDIA:<path>
...
mayros nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
mayros nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
...
- `mayros nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
mayros nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
