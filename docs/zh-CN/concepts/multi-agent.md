...
- **工作区**（文件、AGENTS.md/MAYROS.md/USER.md、本地笔记、人设规则）。
- **状态目录**（`agentDir`）用于认证配置文件、模型注册表和每智能体配置。
- **会话存储**（聊天历史 + 路由状态）位于 `~/.mayros/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.mayros/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.mayros`（或 `MAYROS_STATE_DIR`）
- 工作区：`~/.mayros/workspace`（或 `~/.mayros/workspace-<agentId>`）
- 智能体目录：`~/.mayros/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.mayros/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，Mayros 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.mayros/workspace`（或当设置了 `MAYROS_PROFILE` 时为 `~/.mayros/workspace-<profile>`）。
- 状态默认为 `~/.mayros/agents/main/agent`。
...
- **不同的人格**（每智能体工作区文件如 `AGENTS.md` 和 `MAYROS.md`）。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.mayros/credentials/whatsapp/personal
