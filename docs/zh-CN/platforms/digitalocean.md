---
read_when:
  - 在 DigitalOcean 上设置 Mayros
  - 寻找便宜的 VPS 托管来运行 Mayros
summary: 在 DigitalOcean 上运行 Mayros（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Mayros

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 Mayros Gateway 网关。
...
- `~/.mayros/` — 配置、凭证、会话数据
- `~/.mayros/workspace/` — 工作区（MAYROS.md、记忆等）
