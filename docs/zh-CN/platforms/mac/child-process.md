...
Gateway 网关；如果无法访问，它会通过外部 `mayros` CLI（无嵌入式运行时）启用 launchd
...
- 应用安装标记为 `ai.mayros.gateway` 的按用户 LaunchAgent
  （使用 `--profile`/`MAYROS_PROFILE` 时为 `ai.mayros.<profile>`；支持旧版 `com.mayros.*`）。
...
`--attach-only`（或 `--no-launchd`）启动它。这会设置 `~/.mayros/disable-launchagent`，
