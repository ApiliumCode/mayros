...
read_when:
  - 启动新的 Mayros 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 Mayros 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — Mayros 个人助手（默认）

## 首次运行（推荐）

Mayros 为智能体使用专用的工作区目录。默认：`~/.mayros/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
- 读取 `MAYROS.md`、`USER.md`、`memory.md`，以及 `memory/` 中的今天和昨天的文件。
- 在回复之前完成此操作。

## Soul（必需）

- `MAYROS.md` 定义身份、语气和边界。保持其更新。
- 如果你更改了 `MAYROS.md`，告知用户。
...
如果你将此工作区视为 Atlas 的"记忆"，请将其设为 git 仓库（最好是私有的），这样 `AGENTS.md` 和你的记忆文件就会被备份。
...
- macOS 应用管理权限（屏幕录制、通知、麦克风）并通过其内置二进制文件暴露 `mayros` CLI。
...
- 脚本编写优先使用 `mayros` CLI；mac 应用处理权限。
...
- 对于浏览器驱动的验证，使用带有 Mayros 管理的 Chrome 配置文件的 `mayros browser`（tabs/status/screenshot）。
- 对于 DOM 检查，使用 `mayros browser eval|query|dom|snapshot`（需要机器输出时使用 `--json`/`--out`）。
- 对于交互，使用 `mayros browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（click/type 需要 snapshot 引用；CSS 选择器使用 `evaluate`）。
