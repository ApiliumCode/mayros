...
read_when:
  - 设计 macOS 新手引导助手
  - 实现认证或身份设置
summary: Mayros 的首次运行新手引导流程（macOS 应用）
title: 新手引导
x-i18n:
...
- 打开浏览器进行 OAuth（PKCE）
- 要求用户粘贴 `code#state` 值
- 将凭证写入 `~/.mayros/credentials/oauth.json`
...
应用可以通过 npm/pnpm 安装全局 `mayros` CLI，以便终端工作流和 launchd 任务开箱即用。
...
在首次智能体运行时，Mayros 会引导一个工作区（默认 `~/.mayros/workspace`）：

- 初始化 `AGENTS.md`、`BOOTSTRAP.md`、`IDENTITY.md`、`USER.md`
- 运行简短的问答仪式（一次一个问题）
- 将身份 + 偏好写入 `IDENTITY.md`、`USER.md`、`MAYROS.md`
...
```bash
mayros webhooks gmail setup --account you@gmail.com
```
