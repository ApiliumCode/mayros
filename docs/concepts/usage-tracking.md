...
- `/usage cost` in chats: local cost summary aggregated from Mayros session logs.
- CLI: `mayros status --usage` prints a full per-provider breakdown.
- CLI: `mayros channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
