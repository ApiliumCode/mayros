---
summary: "Mayros on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Mayros on a Raspberry Pi
  - Running Mayros on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Mayros on Raspberry Pi

## Goal

Run a persistent, always-on Mayros Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Mayros features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled mayros

# Enable if not
sudo systemctl enable mayros

# Start on boot
sudo systemctl start mayros
```
...
```bash
# Check logs
journalctl -u mayros --no-pager -n 100

# Common fix: rebuild
cd ~/mayros  # if using hackable install
npm run build
sudo systemctl restart mayros
```
