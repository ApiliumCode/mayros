...
- Same path: run the app from a fixed location (for Mayros, `dist/Mayros.app`).
...
```bash
sudo tccutil reset Accessibility ai.mayros.mac
sudo tccutil reset ScreenCapture ai.mayros.mac
...
Workaround: move files into the Mayros workspace (`~/.mayros/workspace`) if you want to avoid per-folder grants.
