...
Mayros integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Mayros owns lifecycle when `channels.signal.autoStart=true`.
...
- Mayros spawns `imsg rpc` as a child process (legacy iMessage integration).
