...
This document explains how Mayros manages sessions end-to-end:
...
Mayros is designed around a single **Gateway process** that owns session state.
...
Mayros persists sessions in two layers:
...
- Transcripts: `~/.mayros/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Mayros resolves these via `src/config/sessions.ts`.
...
Mayros intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Mayros consumes the events, but Pi decides when to compact).
...
Mayros also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Mayros bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Mayros leaves it alone.
...
Mayros supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Mayros strips/suppresses this in the delivery layer.

As of `2026.1.10`, Mayros also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Mayros uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Mayros’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `mayros status`.
