...
Mayros features that can generate provider usage or paid API calls.
...
- `mayros status --usage` and `mayros channels list` show provider **usage windows**
...
Mayros can pick up credentials from:
...
`mayros models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
