...
read_when:
  - Running Mayros behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Mayros
...
3. Mayros checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Mayros extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://mayros.example.com
    to: http://mayros-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
3. Update Mayros config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `mayros security audit` and review findings
