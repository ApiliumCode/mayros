...
## Quick check: `mayros security audit`
...
Mayros is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Mayros assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.mayros`, including `mayros.json`), treat them as a trusted operator.
...
If you run `--deep`, Mayros also attempts a best-effort live Gateway probe.
...
- **Pairing allowlists**: `~/.mayros/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.mayros/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.mayros/credentials/oauth.json`
...
| --------------------------------------------- | ------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`           | critical      | Other users/processes can modify full Mayros state                      | filesystem perms on `~/.mayros`                               | yes      |
| `fs.config.perms_writable`                    | critical      | Others can change auth/tool policy/config                               | filesystem perms on `~/.mayros/mayros.json`                   | yes      |
...
`mayros security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`mayros security audit` includes `config.insecure_or_dangerous_flags` when any
...
Mayros stores session transcripts on disk under `~/.mayros/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.mayros` (see the audit section below). If you need
...
Mayros can refresh the skills list mid-session:
...
- If you install plugins from npm (`mayros plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.mayros/extensions/<pluginId>/` (or `$MAYROS_STATE_DIR/extensions/<pluginId>/`).
  - Mayros uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
By default, Mayros routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Mayros has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.mayros/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.mayros or your logs.”

## Unsafe external content bypass flags

Mayros includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.mayros/mayros.json`: `600` (user read/write only)
- `~/.mayros`: `700` (user only)

`mayros doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `MAYROS_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__mayros__/canvas/` and `/__mayros__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_mayros-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `MAYROS_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `mayros doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `MAYROS_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `MAYROS_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Mayros
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Mayros verifies the identity by resolving the
...
- Mayros will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.mayros/` (or `$MAYROS_STATE_DIR/`) may contain secrets or private data:

- `mayros.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `mayros status --all` (pasteable, secrets redacted) over raw logs.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.mayros/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `mayros` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Mayros clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Mayros limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `mayros gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `MAYROS_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/mayros/mayros-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.mayros/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `mayros security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Mayros version
...
Found a vulnerability in Mayros? Please report responsibly:

1. Email: [security@apilium.com](mailto:security@apilium.com)
