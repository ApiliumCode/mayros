...
description: "Complete field-by-field reference for ~/.mayros/mayros.json"
---

# Configuration Reference

Every field available in `~/.mayros/mayros.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Mayros uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `mayros doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dmPolicy: "pairing",
      allowFrom: ["1234567890", "testuser"],
      dm: { enabled: true, groupEnabled: false, groupChannels: ["mayros-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `mayros plugins install @apilium/mayros-mattermost`.
...
Mayros spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@mayros"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `mayros.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Mayros auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Mayros assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "mayros-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.mayros/sandboxes`
...
noVNC observer access uses VNC auth by default and Mayros emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `mayros-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.mayros/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@mayros"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:mayros`     | All built-in tools (excludes provider plugins)                                           |
...
Mayros uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.mayros/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `MAYROS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `mayros onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `mayros onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `mayros onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `mayros onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `mayros onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `mayros onboard --auth-choice minimax-api`.
...
- Loaded from `~/.mayros/extensions`, `<workspace>/.mayros/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or MAYROS_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `MAYROS_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.mayros-dev` + port `19001`), `--profile <name>` (uses `~/.mayros-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.mayros/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-mayros-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `MAYROS_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or MAYROS_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__mayros__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__mayros__/a2ui/`
...
- Also serves A2UI at `/__mayros__/a2ui/`.
...
- Hostname defaults to `mayros`. Override with `MAYROS_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.mayros/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `mayros dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.mayros/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${MAYROS_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.mayros/credentials/oauth.json`.
...
- Default log file: `/tmp/mayros/mayros-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `mayros doctor --fix` can strip unknown keys).
