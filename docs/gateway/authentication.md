...
Mayros supports OAuth and API keys for model providers. For Anthropic
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.mayros/.env` so the daemon can read it:
...
API keys for daemon use: `mayros onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.mayros/.env`, systemd/launchd).
...
- Mayros retries with the next key only for rate-limit errors (for example
...
Run `mayros models status` to confirm which profile is expiring. If the profile
