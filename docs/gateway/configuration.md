...
read_when:
  - Setting up Mayros for the first time
...
Mayros reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.mayros/mayros.json`.
...
<Tip>
**New to configuration?** Start with `mayros onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  ...
  <Tab title="Direct edit">
    Edit `~/.mayros/mayros.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `mayros config set` for single keys.
    </Warning>
    ...
    mayros gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.mayros/workspace\" } } }",
      "baseHash": "<hash>",
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Mayros runs your login shell and imports only the missing keys:
...
Env var equivalent: `MAYROS_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${MAYROS_GATEWAY_TOKEN}" } },
