...
- During heartbeat runs, Mayros treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `mayros sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Mayros skips the heartbeat run entirely (no model call).
...
```bash
mayros system event --text "Check for urgent follow-ups" --mode now
```
