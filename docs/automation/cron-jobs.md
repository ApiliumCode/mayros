...
- Jobs persist under `~/.mayros/cron/` so restarts don’t lose schedules.
...
mayros cron list
mayros cron run <job-id>
mayros cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.mayros/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `mayros cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Mayros applies a
...
If `delivery` is omitted for isolated jobs, Mayros defaults to `announce`.
...
- Job store: `~/.mayros/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.mayros/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
mayros cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
mayros cron edit <jobId> --agent ops
mayros cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
mayros cron run <jobId>
mayros cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
mayros cron edit <jobId> \
...
```bash
mayros system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`mayros system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `MAYROS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Mayros applies exponential retry backoff for recurring jobs after consecutive errors:
