...
read_when:
  ...
  - Wiring external systems into Mayros
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Mayros config directory (typically `~/.mayros/hooks/transforms`).
...
- `mayros webhooks gmail setup` writes `hooks.gmail` config for `mayros webhooks gmail run`.
