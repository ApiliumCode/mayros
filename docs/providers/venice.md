---
summary: "Use Venice AI privacy-focused models in Mayros"
read_when:
  - You want privacy-focused inference in Mayros
...
```bash
mayros onboard --auth-choice venice-api-key
```
...
```bash
mayros chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Mayros shows all available Venice models. Pick based on your needs:
...
```bash
mayros models set venice/claude-opus-45
mayros models set venice/llama-3.3-70b
```

List all available models:

```bash
mayros models list | grep venice
```

## Configure via `mayros configure`

1. Run `mayros configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Mayros automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
mayros chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
mayros chat --model venice/claude-opus-45

# Use uncensored model
mayros chat --model venice/venice-uncensored

# Use vision model with image
mayros chat --model venice/qwen3-vl-235b-a22b

# Use coding model
mayros chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `mayros models list` to see currently available models. Some models may be temporarily offline.
