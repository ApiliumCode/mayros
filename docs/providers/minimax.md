---
summary: "Use MiniMax M2.1 in Mayros"
read_when:
  - You want MiniMax models in Mayros
...
```bash
mayros plugins enable minimax-portal-auth  # skip if already loaded.
mayros gateway restart  # restart if gateway is already running
mayros onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/ApiliumCode/mayros/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `mayros.json`:
...
- Use `mayros models list` and `mayros models set minimax/MiniMax-M2.1` to switch.
...
- Running `mayros configure` and selecting **MiniMax M2.1**, or
