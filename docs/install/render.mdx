...
Deploy Mayros on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/ApiliumCode/mayros)
...
repository configures everything needed to run Mayros:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: MAYROS_STATE_DIR
        value: /data/.mayros
      - key: MAYROS_WORKSPACE_DIR
        value: /data/workspace
      - key: MAYROS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/mayros`.
...
If you use the original Mayros repository, Render will not auto-deploy your Mayros. To update it, run a manual Blueprint sync from the dashboard.
...
For Mayros, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Mayros host.
