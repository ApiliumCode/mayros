...
Deploy Mayros on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/mayros-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/mayros` — Control UI

## What you get

- Hosted Mayros Gateway + Control UI
...
- `MAYROS_STATE_DIR=/data/.mayros` (recommended)
- `MAYROS_WORKSPACE_DIR=/data/workspace` (recommended)
- `MAYROS_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Mayros state + workspace so you can migrate to another host without losing config or memory.
