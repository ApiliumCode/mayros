...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/mayros`)
...
If the page fails to load, start the Gateway first: `mayros gateway`.
...
you revoke it with `mayros devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.mayros/mayros.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Mayros
...
Mayros **blocks** Control UI connections without device identity.
