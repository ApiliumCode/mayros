...
Mayros extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
mayros message send --channel zalouser --target <threadId> --message "Hello from Mayros!"
```
...
```bash
mayros channels login --channel zalouser              # Login via QR
mayros channels login --channel zalouser --account work
...
```bash
mayros directory self --channel zalouser
mayros directory peers list --channel zalouser --query "name"
mayros directory groups list --channel zalouser --query "work"
mayros directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
mayros message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
mayros message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `mayros channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
mayros channels login --channel zalouser --account work
mayros message send --channel zalouser --account work --target <id> --message "Hello"
