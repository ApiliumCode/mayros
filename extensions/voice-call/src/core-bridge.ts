...
function resolveMayrosRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.MAYROS_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set MAYROS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveMayrosRoot(), "dist", "extensionAPI.js");
