 ...
 */
function setTwitchAccount(cfg: MayrosConfig, account: Partial<TwitchAccountConfig>): MayrosConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: MAYROS_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: MayrosConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var MAYROS_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.MAYROS_TWITCH_ACCESS_TOKEN?.trim();
