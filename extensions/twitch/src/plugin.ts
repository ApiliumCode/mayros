/**
 * Twitch channel plugin for Mayros.
 *
...
import { buildChannelConfigSchema } from "mayros/plugin-sdk";
 ...
 * for Mayros. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: MayrosConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: MayrosConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: MayrosConfig): boolean => {
