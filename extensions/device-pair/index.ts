...
import { approveDevicePairing, listDevicePairing } from "mayros/plugin-sdk";
...
function resolveGatewayPort(cfg: MayrosPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.MAYROS_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.MAYROS_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: MayrosPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: MayrosPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Mayros iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Mayros iOS app:",
            "",
            "```",
            qrAscii,
            "```",
