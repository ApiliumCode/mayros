...
metadata: { "mayros": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Mayros `sessions_spawn`
- **File I/O** == Mayros `read`/`write`
- **Remote fetch** == Mayros `web_fetch` (or `exec` with curl when POST is required)
