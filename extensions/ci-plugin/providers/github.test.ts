...
describe("GitHubProvider", () => {
  ...
  it("sets auth header correctly", async () => {
    mf().mockResolvedValue(jsonResponse({ total_count: 0, workflow_runs: [] }));
  ...
  it("listRuns parses GitHub API response", async () => {
    mf().mockResolvedValue(jsonResponse({ total_count: 1, workflow_runs: [makeRun()] }));
  ...
  it("getRun maps status/conclusion correctly", async () => {
    mf().mockResolvedValue(jsonResponse(makeRun({ status: "completed", conclusion: "failure" })));
  ...
  it("maps queued status", async () => {
    mf().mockResolvedValue(jsonResponse(makeRun({ status: "queued", conclusion: null })));
  ...
  it("maps in_progress status", async () => {
    mf().mockResolvedValue(jsonResponse(makeRun({ status: "in_progress", conclusion: null })));
  ...
  it("maps cancelled conclusion", async () => {
    mf().mockResolvedValue(jsonResponse(makeRun({ status: "completed", conclusion: "cancelled" })));
  ...
  it("triggerRun sends correct payload", async () => {
    ...
    const [url, init] = mf().mock.calls[0];
  ...
  it("cancelRun returns true on success", async () => {
    mf().mockResolvedValue({ ok: true, status: 202 } as Response);
  ...
  it("cancelRun returns false on failure", async () => {
    mf().mockResolvedValue({ ok: false, status: 409 } as Response);
  ...
  it("getRunLogs returns text", async () => {
    mf().mockResolvedValue(jsonResponse("log output line 1\nlog output line 2"));
  ...
  it("uses custom baseUrl for GitHub Enterprise", async () => {
    ...
    const calledUrl = mf().mock.calls[0][0] as string;
    expect(calledUrl).toContain("https://ghe.corp.com/api/v3");
  });
  ...
  it("getRun returns completedAt for completed runs", async () => {
    mf().mockResolvedValue(jsonResponse(makeRun()));
