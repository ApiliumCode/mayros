...
import type { MayrosPluginApi, MayrosPluginService } from "mayros/plugin-sdk";
...
function normalizeDenyList(cfg: MayrosPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: MayrosPluginApi["config"]): string[] {
...
export default function register(api: MayrosPluginApi) {
  ...
  const timerService: MayrosPluginService = {
