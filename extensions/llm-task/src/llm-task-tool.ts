...
import Ajv from "ajv";
// NOTE: This extension is intended to be bundled with Mayros.
// When running from source (tests/dev), Mayros internals live under src/.
...
export function createLlmTaskTool(api: MayrosPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via mayros.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-llm-task-"));
