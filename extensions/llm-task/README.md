...
`mayros.invoke --each`) without adding new Mayros code per workflow.
...
This extension depends on Mayros internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Mayros extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.mayros/extensions` as a standalone plugin directory.
