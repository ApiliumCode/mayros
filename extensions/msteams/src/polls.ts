...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["mayrosPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["mayros", "pollId"]) ??
    readNestedString(value, ["mayros", "poll", "id"]) ??
    readNestedString(value, ["data", "mayrosPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "mayros", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "mayros poll vote",
          displayText: "Vote recorded",
          value: { mayrosPollId: pollId, pollId },
        },
      },
    ],
  };
