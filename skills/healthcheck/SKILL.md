---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Mayros deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Mayros cron scheduling for periodic checks, or version status checks on a machine running Mayros (laptop, workstation, Pi, VPS).
---

# Mayros Host Hardening

## Overview

Assess and harden the host running Mayros, then align it to a user-defined risk tolerance without breaking access. Use Mayros security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Mayros changes the host firewall, SSH, or OS updates; it does not.
...
5. Mayros gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Mayros can access sensitive data.
...
### 2) Run Mayros security audits (read-only)

As part of the default read-only checks, run `mayros security audit --deep`. Only offer alternatives if the user requests them:

1. `mayros security audit` (faster, non-probing)
2. `mayros security audit --json` (structured output)

Offer to apply Mayros safe defaults (numbered):

1. `mayros security audit --fix`

Be explicit that `--fix` only tightens Mayros defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Mayros version/update status (read-only)

As part of the default read-only checks, run `mayros update status`.
...
- Credential hygiene notes (location of Mayros creds, prefer disk encryption)
...
After Mayros install or first hardening pass, run at least one baseline audit and version check:

- `mayros security audit`
- `mayros security audit --deep`
- `mayros update status`

Ongoing monitoring is recommended. Use the Mayros cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `mayros cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `mayros update status`
...
Before creating, `mayros cron list` and match on exact `name`. If found, `mayros cron edit <id> ...`.
If not found, `mayros cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `mayros update status` (preferred for source checkouts and channels)
2. `npm view mayros version` (published npm version)

## Mayros command accuracy

Use only supported commands and flags:

- `mayros security audit [--deep] [--fix] [--json]`
- `mayros status` / `mayros status --deep`
- `mayros health --json`
- `mayros update status`
- `mayros cron add|list|runs|run`

Do not invent CLI flags or imply Mayros enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Mayros compaction:
