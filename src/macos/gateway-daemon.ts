...
declare const __MAYROS_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __MAYROS_VERSION__ === "string" && __MAYROS_VERSION__) ||
  process.env.MAYROS_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `mayros --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.MAYROS_GATEWAY_PORT ??
    process.env.MAYROS_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.MAYROS_GATEWAY_BIND ??
    process.env.MAYROS_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (MAYROS_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
