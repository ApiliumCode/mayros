...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { MAYROS_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MAYROS_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports MAYROS_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { MAYROS_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MAYROS_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { MAYROS_SMOKE: "nope" })).toBe("qr");
  });
});
