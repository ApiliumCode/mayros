...
export const DEFAULT_LOG_DIR = resolvePreferredMayrosTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "mayros.log"); // legacy single-file path

const LOG_PREFIX = "mayros";
...
function resolveSettings(): ResolvedSettings {
  let cfg: MayrosConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => MayrosConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.MAYROS_TEST_FILE_LOG !== "1" ? "silent" : "info";
