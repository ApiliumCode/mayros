...
describe("canvas host state dir defaults", () => {
  it("uses MAYROS_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("mayros-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Mayros Canvas");
      } finally {
        await handler.close();
      }
    });
