...
describe("shared/frontmatter", () => {
  ...
  test("resolveMayrosManifestBlock parses JSON5 metadata and picks mayros block", () => {
    const frontmatter = {
      metadata: "{ mayros: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveMayrosManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveMayrosManifestBlock returns undefined for invalid input", () => {
    expect(resolveMayrosManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(resolveMayrosManifestBlock({ frontmatter: { metadata: "not-json5" } })).toBeUndefined();
    expect(
      resolveMayrosManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
