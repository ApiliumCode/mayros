...
function createGroqAudioConfig(): MayrosConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): MayrosConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
async function createMediaTempDir() {
  return await fs.mkdtemp(path.join(resolvePreferredMayrosTmpDir(), "mayros-media-"));
}
