...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["MAYROS_BASE_ENV"]);
    process.env.MAYROS_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.MAYROS_BASE_ENV ?? "") + "|" + (process.env.MAYROS_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { MAYROS_TEST_ENV: "ok" },
        },
      );
