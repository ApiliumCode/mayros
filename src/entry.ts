...
function ensureExperimentalWarningSuppressed(): boolean {
  ...
  if (isTruthyEnvValue(process.env.MAYROS_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.MAYROS_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.MAYROS_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[mayros] ${parsed.error}`);
    process.exit(2);
  }
