...
function resolveGatewayPort(cfg: MayrosConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: MayrosConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.MAYROS_GATEWAY_PASSWORD?.trim() ||
    env.MAYROS_GATEWAY_PASSWORD?.trim() ||
