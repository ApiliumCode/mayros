...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mayros-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "mayros-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "mayros-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "mayros-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.mayros when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mayros-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers MAYROS_HOME over HOME", () => {
    vi.stubEnv("MAYROS_HOME", "/srv/mayros-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/mayros-home"));
...
describe("shortenHomePath", () => {
  it("uses $MAYROS_HOME prefix when MAYROS_HOME is set", () => {
    vi.stubEnv("MAYROS_HOME", "/srv/mayros-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/mayros-home")}/.mayros/mayros.json`)).toBe(
      "$MAYROS_HOME/.mayros/mayros.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $MAYROS_HOME replacement when MAYROS_HOME is set", () => {
    vi.stubEnv("MAYROS_HOME", "/srv/mayros-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/mayros-home")}/.mayros/mayros.json`),
    ).toBe("config: $MAYROS_HOME/.mayros/mayros.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/mayros")).toBe(path.resolve(os.homedir(), "mayros"));
  });
  ...
  it("prefers MAYROS_HOME for tilde expansion", () => {
    vi.stubEnv("MAYROS_HOME", "/srv/mayros-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/mayros")).toBe(path.resolve("/srv/mayros-home", "mayros"));
