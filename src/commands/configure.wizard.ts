...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.MAYROS_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.MAYROS_GATEWAY_PASSWORD;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from mayros.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Mayros update wizard" : "Mayros configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: MayrosConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://apilium.com/us/doc/mayros/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("mayros doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.MAYROS_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.MAYROS_GATEWAY_PASSWORD,
    });
    ...
    let gatewayToken: string | undefined =
      ...
      process.env.MAYROS_GATEWAY_TOKEN;
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.MAYROS_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.MAYROS_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.MAYROS_GATEWAY_TOKEN;
