...
describe("doctor command", () => {
  ...
  it("does not warn when only the active workspace is present", async () => {
    mockDoctorConfigSnapshot({
      config: {
        agents: { defaults: { workspace: "/Users/testuser/mayros" } },
      },
    });

    note.mockClear();
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/Users/testuser");
    const realExists = fs.existsSync;
    const legacyPath = path.join("/Users/testuser", "mayros");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (
        value === "/Users/testuser/mayros" ||
        value === legacyPath ||
