...
function resolveAgentName(cfg: MayrosConfig, agentId: string) {
...
function resolveAgentModel(cfg: MayrosConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MayrosConfig): AgentSummary[] {
