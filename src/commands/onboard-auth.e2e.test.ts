...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under MAYROS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("mayros-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-oauth-sync-"));
    ...
    process.env.MAYROS_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-oauth-nosync-"));
    process.env.MAYROS_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside MAYROS_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-oauth-external-"));
    process.env.MAYROS_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* MAYROS_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MAYROS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("mayros-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
