...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "MAYROS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("MAYROS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv MAYROS_GATEWAY_TOKEN");
    expect(message).not.toContain("MAYROS_GATEWAY_PASSWORD");
  });
