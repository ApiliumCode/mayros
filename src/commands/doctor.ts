...
function resolveMode(cfg: MayrosConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: MayrosConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("mayros configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("mayros config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("mayros setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("mayros doctor --fix")}" to apply changes.`);
  }
