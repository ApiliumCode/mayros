...
import type { MayrosConfig, GatewayBindMode } from "../config/config.js";
import { resolveGatewayAuth } from "../gateway/auth.js";
import { isLoopbackHost, resolveGatewayBindHost } from "../gateway/net.js";
import { resolveDmAllowState } from "../security/dm-policy-shared.js";
import { note } from "../terminal/note.js";

export async function noteSecurityWarnings(cfg: MayrosConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("mayros security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("mayros configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("mayros config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("mayros doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("mayros config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('mayros config set session.dmScope "per-channel-peer"') +
