...
beforeAll(() => {
  envSnapshot = captureEnv(["MAYROS_PROFILE"]);
  process.env.MAYROS_PROFILE = "isolated";
});
...
vi.mock("../infra/mayros-root.js", () => ({
  resolveMayrosPackageRoot: vi.fn().mockResolvedValue("/tmp/mayros"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/mayros/pnpm-lock.yaml",
      markerPath: "/tmp/mayros/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.mayros.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.mayros.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Mayros status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("mayros status --all") ||
          l.includes("mayros --profile isolated status --all") ||
          l.includes("mayros status --all") ||
          l.includes("mayros --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.MAYROS_GATEWAY_TOKEN = "abcd1234";
    try {
