...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMayrosAgentDir: vi.fn().mockReturnValue("/tmp/mayros-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/mayros-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/mayros-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveMayrosAgentDir: mocks.resolveMayrosAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/mayros-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveMayrosAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/mayros-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/mayros-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/mayros-agent-custom");
