...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveMayrosAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("mayros models auth setup-token")}\` or \`${formatCliCommand("mayros configure")}\`.`
          : `Run \`${formatCliCommand("mayros configure")}\` or set an API key env var.`;
