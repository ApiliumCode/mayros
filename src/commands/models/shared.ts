...
import { type MayrosConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function loadValidConfigOrThrow(): Promise<MayrosConfig> {
...
export async function updateConfig(
  mutator: (cfg: MayrosConfig) => MayrosConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: MayrosConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: MayrosConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("mayros agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
