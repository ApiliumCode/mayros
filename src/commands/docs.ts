...
const SEARCH_TOOL = "https://apilium.com/us/doc/mayros/mcp.SearchMayros";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "apilium.com/us/doc/mayros");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('mayros docs "your query"')}`);
    } else {
      runtime.log("Docs: https://apilium.com/us/doc/mayros/");
      runtime.log(`Search: ${formatCliCommand('mayros docs "your query"')}`);
    }
    return;
  }
