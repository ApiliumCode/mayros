...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".mayros-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + mayros-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateMayrosProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MAYROS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MAYROS_BROWSER_COLOR).toUpperCase();
