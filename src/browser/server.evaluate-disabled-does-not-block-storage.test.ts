  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          mayros: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MAYROS_GATEWAY_PORT;
    process.env.MAYROS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MAYROS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MAYROS_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayToken === undefined) {
      ...
      process.env.MAYROS_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.MAYROS_GATEWAY_PASSWORD;
    } else {
      process.env.MAYROS_GATEWAY_PASSWORD = prevGatewayPassword;
    }
