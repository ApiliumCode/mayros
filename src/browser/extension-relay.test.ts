...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    previousGatewayToken = process.env.MAYROS_GATEWAY_TOKEN;
    process.env.MAYROS_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
  });
  ...
  afterEach(async () => {
    ...
    if (previousGatewayToken === undefined) {
      ...
      process.env.MAYROS_GATEWAY_TOKEN = previousGatewayToken;
    }
  });
  ...
  it("uses gateway token for relay auth headers on loopback URLs", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-mayros-relay-token");
    expect(headers["x-mayros-relay-token"]).toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    process.env.MAYROS_GATEWAY_TOKEN = "test-gateway-token";
    try {
      cdpUrl = `http://127.0.0.1:${port}`;
      ...
      if (prev === undefined) {
        ...
        process.env.MAYROS_GATEWAY_TOKEN = prev;
      }
      await new Promise<void>((resolve) => fakeRelay.close(() => resolve()));
    }
  });

  it("does not swallow EADDRINUSE when occupied port is not an mayros relay", async () => {
    const port = await getFreePort();
