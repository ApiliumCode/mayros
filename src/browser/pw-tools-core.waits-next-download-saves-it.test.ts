...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredMayrosTmpDir: vi.fn(() => "/tmp/mayros"),
}));
vi.mock("../infra/tmp-mayros-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredMayrosTmpDir.mockReturnValue("/tmp/mayros");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredMayrosTmpDir.mockReturnValue("/tmp/mayros-preferred");
    const { res, outPath } = await waitForImplicitDownloadOutput({
      downloadUrl: "https://example.com/file.bin",
      suggestedFilename: "file.bin",
    });
    expect(typeof outPath).toBe("string");
    const expectedRootedDownloadsDir = path.join(path.sep, "tmp", "mayros-preferred", "downloads");
    const expectedDownloadsTail = `${path.join("tmp", "mayros-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredMayrosTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "mayros-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "mayros-preferred", "downloads")}${path.sep}`),
    );
  });
