...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and mayros-orange color", () => {
    ...
    const mayros = resolveProfile(resolved, "mayros");
    expect(mayros?.driver).toBe("mayros");
    expect(mayros?.cdpPort).toBe(18800);
    expect(mayros?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from MAYROS_GATEWAY_PORT when unset", () => {
    const prev = process.env.MAYROS_GATEWAY_PORT;
    process.env.MAYROS_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const mayros = resolveProfile(resolved, "mayros");
      expect(mayros?.cdpPort).toBe(19012);
      expect(mayros?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.MAYROS_GATEWAY_PORT;
      } else {
        process.env.MAYROS_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const mayros = resolveProfile(resolved, "mayros");
      expect(mayros?.cdpPort).toBe(19022);
      expect(mayros?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.MAYROS_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "mayros");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        mayros: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("mayros");
  });
