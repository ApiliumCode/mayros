import type { BrowserConfig, BrowserProfileConfig, MayrosConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_MAYROS_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "mayros" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_MAYROS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "mayros" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MAYROS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MAYROS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Mayros browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for mayros/mayros CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: MayrosConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_MAYROS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "mayros";
