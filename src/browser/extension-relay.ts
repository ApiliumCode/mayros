...
const RELAY_AUTH_HEADER = "x-mayros-relay-token";
...
function resolveGatewayAuthToken(): string | null {
  const envToken = process.env.MAYROS_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
function resolveRelayAuthToken(): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or MAYROS_GATEWAY_TOKEN)",
  );
}
...
async function looksLikeMayrosRelay(baseUrl: string): Promise<boolean> {
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Mayros-Extension-Relay",
          revision: "0",
          userAgent: "Mayros-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "Mayros/extension-relay",
  ...
  try {
    ...
    if (isAddrInUseError(err) && (await looksLikeMayrosRelay(info.baseUrl))) {
