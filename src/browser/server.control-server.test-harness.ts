...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.MAYROS_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/mayros" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "mayros",
        profiles: {
          mayros: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchMayrosChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.MAYROS_GATEWAY_PORT;
    process.env.MAYROS_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.MAYROS_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.MAYROS_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayToken === undefined) {
      ...
      process.env.MAYROS_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.MAYROS_GATEWAY_PASSWORD;
    } else {
      process.env.MAYROS_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
