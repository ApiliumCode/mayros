  ...
  it.each([
    {
      ...
      expectedPackId: "mayros-zip-hooks",
    ...
    },
    {
      ...
      expectedPackId: "mayros-tar-hooks",
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@apilium/mayros-test-hooks",
        version: "0.0.1",
        mayros: { hooks: ["./hooks/one-hook"] },
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expect(result.error).toContain("mayros.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expect(result.error).toContain("mayros.hooks entry resolves outside package directory");
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@apilium/mayros-test-hooks@0.0.1",
              name: "@apilium/mayros-test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@apilium/mayros-test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.hookPackId).toBe("mayros-test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@apilium/mayros-test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@apilium/mayros-test-hooks@0.0.1",
    });
