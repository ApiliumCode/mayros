...
This directory contains hooks that ship with Mayros. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.mayros/mayros.json`:
...
2. Restart gateway: `pkill -9 -f 'mayros.*gateway' && pnpm mayros gateway`
3. Enable the hook: `mayros hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://apilium.com/us/doc/mayros/automation/hooks
