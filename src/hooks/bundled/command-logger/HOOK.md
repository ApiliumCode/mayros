...
homepage: https://apilium.com/us/doc/mayros/automation/hooks#command-logger
metadata:
  {
    "mayros":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.mayros/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.mayros/logs/commands.log ~/.mayros/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/mayros`:
   ```
   /home/username/.mayros/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
