...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => MayrosConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("mayros-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): MayrosConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
