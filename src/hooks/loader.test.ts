...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.MAYROS_BUNDLED_HOOKS_DIR;
    process.env.MAYROS_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.MAYROS_BUNDLED_HOOKS_DIR;
    } else {
      process.env.MAYROS_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
