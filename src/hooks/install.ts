...
async function ensureMayrosHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing mayros.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json mayros.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureMayrosHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `mayros.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `mayros.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromArchive(params: {
  ...
  return await withTempDir("mayros-hook-", async (tmpDir) => {
...
export async function installHooksFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchive({
    tempDirPrefix: "mayros-hook-pack-",
    spec,
    timeoutMs,
