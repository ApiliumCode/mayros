...
function resolveExecConfig(params: { cfg?: MayrosConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: MayrosConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createMayrosCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createMayrosReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
