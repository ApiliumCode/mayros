...
const defaultTools = createMayrosCodingTools({ senderIsOwner: true });
...
describe("createMayrosCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createMayrosCodingTools({
      config,
    ...
    const anthropicTools = createMayrosCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createMayrosCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createMayrosCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
