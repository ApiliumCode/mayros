...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MAYROS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MAYROS_HOME: "/srv/mayros-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/mayros-home"), ".mayros", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".mayros", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("mayros-workspace-");
