...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "mayros.sandbox=1");
  args.push("--label", `mayros.sessionKey=${params.scopeKey}`);
  args.push("--label", `mayros.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `mayros.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "mayros.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`mayros sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`mayros sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("mayros sandbox recreate --all");
}
