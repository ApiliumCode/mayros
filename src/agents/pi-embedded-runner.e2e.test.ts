...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-embedded-agent-"));
...
const makeOpenAiConfig = (modelIds: string[]) =>
  ...
  }) satisfies MayrosConfig;

const ensureModels = (cfg: MayrosConfig) => ensureMayrosModelsJson(cfg, agentDir) as unknown;
...
describe("runEmbeddedPiAgent", () => {
  ...
  it("persists prompt transport errors as transcript entries", async () => {
    ...
    const promptErrorEntry = entries.find(
      (entry) => entry.type === "custom" && entry.customType === "mayros:prompt-error",
