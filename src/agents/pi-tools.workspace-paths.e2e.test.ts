...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("mayros-ws-", async (workspaceDir) => {
      await withTempDir("mayros-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createMayrosCodingTools({ workspaceDir });
    ...
    await withTempDir("mayros-ws-", async (workspaceDir) => {
      await withTempDir("mayros-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello mayros");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("mayros-ws-", async (workspaceDir) => {
      await withTempDir("mayros-override-", async (overrideDir) => {
        const tools = createMayrosCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("mayros-sandbox-", async (sandboxDir) => {
      await withTempDir("mayros-workspace-", async (workspaceDir) => {
        ...
        const tools = createMayrosCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
