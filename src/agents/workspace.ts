...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.MAYROS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".mayros", `workspace-${profile}`);
  }
  return path.join(home, ".mayros", "workspace");
}

export const DEFAULT_AGENT_WORKSPACE_DIR = resolveDefaultAgentWorkspaceDir();
export const DEFAULT_AGENTS_FILENAME = "AGENTS.md";
export const DEFAULT_SOUL_FILENAME = "MAYROS.md";
export const LEGACY_SOUL_FILENAME = "SOUL.md";
export const DEFAULT_TOOLS_FILENAME = "TOOLS.md";
export const DEFAULT_IDENTITY_FILENAME = "IDENTITY.md";
export const DEFAULT_USER_FILENAME = "USER.md";
export const DEFAULT_HEARTBEAT_FILENAME = "HOPE.md";
export const LEGACY_HEARTBEAT_FILENAME = "HEARTBEAT.md";
export const DEFAULT_BOOTSTRAP_FILENAME = "BOOTSTRAP.md";
export const DEFAULT_MEMORY_FILENAME = "MEMORY.md";
export const DEFAULT_MEMORY_ALT_FILENAME = "memory.md";
const WORKSPACE_STATE_DIRNAME = ".mayros";
...
export type WorkspaceBootstrapFileName =
  ...
  | typeof LEGACY_SOUL_FILENAME
  | typeof DEFAULT_TOOLS_FILENAME
  | typeof DEFAULT_IDENTITY_FILENAME
  | typeof DEFAULT_USER_FILENAME
  | typeof DEFAULT_HEARTBEAT_FILENAME
  | typeof LEGACY_HEARTBEAT_FILENAME
...
export async function ensureAgentWorkspace(params?: {
  ...
  // Also check legacy filenames (SOUL.md, HEARTBEAT.md) for brand-new detection
  const legacySoulPath = path.join(dir, LEGACY_SOUL_FILENAME);
  const legacyHeartbeatPath = path.join(dir, LEGACY_HEARTBEAT_FILENAME);
...
export async function loadWorkspaceBootstrapFiles(dir: string): Promise<WorkspaceBootstrapFile[]> {
  ...
  // Backward-compat fallbacks: MAYROS.md → SOUL.md, HOPE.md → HEARTBEAT.md
  const legacyFallbacks: Record<string, string> = {
    [DEFAULT_SOUL_FILENAME]: LEGACY_SOUL_FILENAME,
    [DEFAULT_HEARTBEAT_FILENAME]: LEGACY_HEARTBEAT_FILENAME,
  };
  ...
  for (const entry of entries) {
    try {
      ...
      // Try legacy fallback filename if available
      const legacyName = legacyFallbacks[entry.name];
      if (legacyName) {
        const legacyPath = path.join(resolvedDir, legacyName);
        try {
          const content = await readFileWithCache(legacyPath);
          result.push({
            name: entry.name,
            path: legacyPath,
            content,
          ...
          // Legacy file also missing, fall through
        }
      }
