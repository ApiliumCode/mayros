...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## Mayros CLI Quick Reference");
    expect(prompt).toContain("mayros gateway restart");
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Mayros docs: /tmp/mayros/docs");
    expect(prompt).toContain(
  ...
  // See: https://github.com/ApiliumCode/mayros/commit/66eec295b894bce8333886cfbca3b960c57c4946
  // Agents should use session_status or message timestamps to determine the date/time.
  // Related: https://github.com/ApiliumCode/mayros/issues/1897
  //          https://github.com/ApiliumCode/mayros/issues/3658
  it("does NOT include a date or time in the system prompt (cache stability)", () => {
    ...
    // https://github.com/ApiliumCode/mayros/issues/3658 for the preferred approach:
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Mayros Self-Update");
  ...
  it("adds persona guidance when a MAYROS.md or SOUL.md file is present", () => {
    const promptMayros = buildAgentSystemPrompt({
      workspaceDir: "/tmp/mayros",
      contextFiles: [{ path: "./MAYROS.md", content: "Persona" }],
    });

    expect(promptMayros).toContain(
    ...
    const promptSoul = buildAgentSystemPrompt({
      workspaceDir: "/tmp/mayros",
      contextFiles: [
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/mayros. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
