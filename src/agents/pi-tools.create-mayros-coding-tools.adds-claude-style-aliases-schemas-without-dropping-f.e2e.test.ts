...
describe("createMayrosCodingTools", () => {
  it("uses workspaceDir for Read tool path resolution", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-ws-"));
    try {
      ...
      const tools = createMayrosCodingTools({ workspaceDir: tmpDir });
  ...
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-alias-"));
    try {
      const tools = createMayrosCodingTools({ workspaceDir: tmpDir });
      const readTool = tools.find((tool) => tool.name === "read");
      const writeTool = tools.find((tool) => tool.name === "write");
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-structured-write-"));
    try {
      const tools = createMayrosCodingTools({ workspaceDir: tmpDir });
      const writeTool = tools.find((tool) => tool.name === "write");
      expect(writeTool).toBeDefined();
      ...
      await writeTool?.execute("tool-structured-write", {
        ...
        content: [
          ...
          { type: "input_text", text: "const root = path.join(process.env.HOME, 'mayros');\n" },
        ],
      });

      const written = await fs.readFile(path.join(tmpDir, "structured-write.js"), "utf8");
      expect(written).toBe(
        "const path = require('path');\nconst root = path.join(process.env.HOME, 'mayros');\n",
      );
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-structured-edit-"));
    try {
