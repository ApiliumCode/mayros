...
const GOOGLE_LIVE_SINGLE_KEY = "MAYROS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MAYROS_LIVE_ANTHROPIC_KEY",
    listVar: "MAYROS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MAYROS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MAYROS_LIVE_${base}_KEY`;
