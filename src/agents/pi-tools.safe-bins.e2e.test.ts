...
const bundledPluginsDirSnapshot = captureEnv(["MAYROS_BUNDLED_PLUGINS_DIR"]);

beforeAll(() => {
  process.env.MAYROS_BUNDLED_PLUGINS_DIR = path.join(
    os.tmpdir(),
...
async function createSafeBinsExecTool(params: {
  ...
  const { createMayrosCodingTools } = await import("./pi-tools.js");
...
describe("createMayrosCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    ...
    const { tmpDir, execTool } = await createSafeBinsExecTool({
      tmpPrefix: "mayros-safe-bins-",
      safeBins: ["echo"],
    });

    const marker = `safe-bins-${Date.now()}`;
    const envSnapshot = captureEnv(["MAYROS_SHELL_ENV_TIMEOUT_MS"]);
    const result = await (async () => {
      try {
        process.env.MAYROS_SHELL_ENV_TIMEOUT_MS = "1000";
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    ...
    const { tmpDir, execTool } = await createSafeBinsExecTool({
      tmpPrefix: "mayros-safe-bins-expand-",
      safeBins: ["head", "wc"],
  ...
  it("does not leak file existence from sort output flags", async () => {
    ...
    const { tmpDir, execTool } = await createSafeBinsExecTool({
      tmpPrefix: "mayros-safe-bins-oracle-",
      safeBins: ["sort"],
  ...
  it("blocks sort output flags from writing files via safeBins", async () => {
    ...
    const { tmpDir, execTool } = await createSafeBinsExecTool({
      tmpPrefix: "mayros-safe-bins-sort-",
      safeBins: ["sort"],
    });
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    ...
    const { tmpDir, execTool } = await createSafeBinsExecTool({
      tmpPrefix: "mayros-safe-bins-redirect-",
      safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    ...
    const { tmpDir, execTool } = await createSafeBinsExecTool({
      tmpPrefix: "mayros-safe-bins-grep-",
      safeBins: ["grep"],
