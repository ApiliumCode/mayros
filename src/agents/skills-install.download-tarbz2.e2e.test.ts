...
async function withTempWorkspace(
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-skills-install-"));
  try {
...
function restoreMayrosStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.MAYROS_STATE_DIR;
    return;
  }
  process.env.MAYROS_STATE_DIR = originalValue;
}

const originalStateDir = process.env.MAYROS_STATE_DIR;

afterEach(() => {
  restoreMayrosStateDir(originalStateDir);
});
