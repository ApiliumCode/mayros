...
} from "./mayros-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateMayrosTools = (typeof import("./mayros-tools.js"))["createMayrosTools"];
type CreateMayrosToolsOpts = Parameters<CreateMayrosTools>[0];

async function getSessionsSpawnTool(opts: CreateMayrosToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createMayrosTools } = await import("./mayros-tools.js");
  const tool = createMayrosTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("mayros-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
