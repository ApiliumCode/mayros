...
describe("getShellConfig", () => {
  ...
  const createTempBin = (files: string[]) => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mayros-shell-"));
    tempDirs.push(dir);
...
describe("resolveShellFromPath", () => {
  ...
  const createTempBin = (name: string, executable: boolean) => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mayros-shell-path-"));
    tempDirs.push(dir);
    const filePath = path.join(dir, name);
    fs.writeFileSync(filePath, "");
    if (executable) {
  ...
  it("returns undefined when command does not exist", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mayros-shell-empty-"));
    tempDirs.push(dir);
