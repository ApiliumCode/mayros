...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const envSnapshot = captureEnv(["MAYROS_STATE_DIR", "MAYROS_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-oauth-"));

    try {
      process.env.MAYROS_STATE_DIR = tempDir;
      process.env.MAYROS_AGENT_DIR = path.join(tempDir, "agent");
      process.env.PI_CODING_AGENT_DIR = process.env.MAYROS_AGENT_DIR;
      ...
      const store = ensureAuthProfileStore(process.env.MAYROS_AGENT_DIR, {
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-auth-"));
