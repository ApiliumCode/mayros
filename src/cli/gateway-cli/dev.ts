...
const DEV_IDENTITY_NAME = "Atlas";
const DEV_IDENTITY_THEME = "knowledge navigator";
const DEV_IDENTITY_EMOJI = "⚛️";
...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = env.MAYROS_PROFILE?.trim().toLowerCase();
  if (profile === "dev") {
    return baseDir;
  }
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - Mayros Dev Workspace\n\nDefault dev workspace for mayros gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",
      `# SOUL.md - Dev Persona\n\nKnowledge navigator for debugging and operations.\n`,
    ),
    loadDevTemplate(
      "TOOLS.dev.md",
