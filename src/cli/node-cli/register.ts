    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mayros node run --host 127.0.0.1 --port 18789", "Run the node host in the foreground."],
          ["mayros node status", "Check node host service status."],
          ["mayros node install", "Install the node host service."],
          ["mayros node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "apilium.com/us/doc/mayros/cli/node")}\n`,
    );
