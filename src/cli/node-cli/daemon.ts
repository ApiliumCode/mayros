...
import { DEFAULT_GATEWAY_PORT, resolveIsNixMode } from "../../config/paths.js";
import {
...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("mayros node install"), formatCliCommand("mayros node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("mayros node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    MAYROS_LOG_PREFIX: baseEnv.MAYROS_LOG_PREFIX ?? "node",
