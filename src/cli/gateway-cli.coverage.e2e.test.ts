...
describe("gateway-cli coverage", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        "Gateway Discovery",
        "Found 1 gateway(s)",
        "- Studio mayros.internal.",
        "  tailnet: studio.tailnet.ts.net",
        "  host: studio.mayros.internal",
        "  ws: ws://studio.mayros.internal:18789",
      ],
    },
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ MAYROS_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
