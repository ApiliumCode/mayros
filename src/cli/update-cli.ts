...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["mayros update", "Update a source checkout (git)"],
        ["mayros update --channel beta", "Switch to beta channel (git + npm)"],
        ["mayros update --channel dev", "Switch to dev channel (git + npm)"],
        ["mayros update --tag beta", "One-off update to a dist-tag or version"],
        ["mayros update --no-restart", "Update without restarting the service"],
        ["mayros update --json", "Output result as JSON"],
        ["mayros update --yes", "Non-interactive (accept downgrade prompts)"],
        ["mayros update wizard", "Interactive update wizard"],
        ["mayros --update", "Shorthand for mayros update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run mayros update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "apilium.com/us/doc/mayros/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mayros update status", "Show channel + version status."],
          ["mayros update status --json", "JSON output."],
          ["mayros update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "apilium.com/us/doc/mayros/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
