...
describe("kaneru subcommands", () => {
  ...
  it("has exactly 15 top-level subcommands under kaneru", () => {
    const program = makeProgram();
    const kaneru = getKaneru(program);
    const names = kaneru.commands.map((c) => c.name()).sort();
    expect(names).toEqual(["consensus", "dashboard", "decisions", "delegate", "dojo", "fuel", "fuse", "learn", "mission", "peers", "pulse", "route", "squad", "sync", "venture"]);
  });
});
