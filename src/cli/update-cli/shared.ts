...
import { resolveMayrosPackageRoot } from "../../infra/mayros-root.js";
...
const MAYROS_REPO_URL = "https://github.com/ApiliumCode/mayros.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "mayros";
...
export function resolveGitInstallDir(): string {
  const override = process.env.MAYROS_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveMayrosPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", MAYROS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MAYROS_GIT_DIR points at a non-git directory: ${params.dir}. Set MAYROS_GIT_DIR to an empty folder or an mayros checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MAYROS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "mayros.mjs");
