...
function appendSkillsHubHint(output: string, json?: boolean): string {
  ...
  return `${output}\n\nTip: use \`npx skillshub\` to search, install, and sync skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("mayros skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendSkillsHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("mayros skills list")}\` to see available skills.`,
      opts.json,
    );
  }
