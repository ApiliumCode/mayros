    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mayros nodes status", "List known nodes with live status."],
          ["mayros nodes pairing pending", "Show pending node pairing requests."],
          ['mayros nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["mayros nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "apilium.com/us/doc/mayros/cli/nodes")}\n`,
    );
