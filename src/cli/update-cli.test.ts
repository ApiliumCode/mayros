...
import type { MayrosConfig, ConfigFileSnapshot } from "../config/types.mayros.js";
...
vi.mock("../infra/mayros-root.js", () => ({
  resolveMayrosPackageRoot: vi.fn(),
}));
...
const { resolveMayrosPackageRoot } = await import("../infra/mayros-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/mayros-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as MayrosConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/mayros-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveMayrosPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("mayros-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveMayrosPackageRoot).mockReset();
    ...
    vi.mocked(resolveMayrosPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("mayros");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/mayros-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "mayros-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Mayros update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as MayrosConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveMayrosPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("mayros-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    const envSnapshot = captureEnv(["MAYROS_UPDATE_IN_PROGRESS"]);
    ...
    try {
      ...
      expect(process.env.MAYROS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("mayros-update-wizard");
    const envSnapshot = captureEnv(["MAYROS_GIT_DIR"]);
    try {
      setTty(true);
      process.env.MAYROS_GIT_DIR = tempDir;
