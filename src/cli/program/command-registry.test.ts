...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "mayros", "doctor"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "mayros", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "mayros", "status"], async () => {
      await program.parseAsync(["node", "mayros", "status"]);
    });
