...
${formatHelpExamples([
  ['mayros agent --to +15555550123 --message "status update"', "Start a new session."],
  ['mayros agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'mayros agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'mayros agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['mayros agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'mayros agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "apilium.com/us/doc/mayros/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "apilium.com/us/doc/mayros/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['mayros agents set-identity --agent main --name "Mayros" --emoji "⚡🛡️"', "Set name + emoji."],
  ["mayros agents set-identity --agent main --avatar avatars/mayros.png", "Set avatar path."],
  [
    "mayros agents set-identity --workspace ~/.mayros/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "mayros agents set-identity --identity-file ~/.mayros/workspace/IDENTITY.md --agent main",
