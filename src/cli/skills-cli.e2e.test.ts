...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["MAYROS_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "mayros-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "mayros-bundled-skills-test-"));
    process.env.MAYROS_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
