...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    MAYROS_STATE_DIR: process.env.MAYROS_STATE_DIR,
    MAYROS_CONFIG_PATH: process.env.MAYROS_CONFIG_PATH,
    MAYROS_GATEWAY_PORT: process.env.MAYROS_GATEWAY_PORT,
    MAYROS_PROFILE: process.env.MAYROS_PROFILE,
  };

  beforeEach(() => {
    process.env.MAYROS_STATE_DIR = "/tmp/mayros-cli-state";
    process.env.MAYROS_CONFIG_PATH = "/tmp/mayros-cli-state/mayros.json";
    delete process.env.MAYROS_GATEWAY_PORT;
    delete process.env.MAYROS_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.MAYROS_STATE_DIR !== undefined) {
      process.env.MAYROS_STATE_DIR = originalEnv.MAYROS_STATE_DIR;
    } else {
      delete process.env.MAYROS_STATE_DIR;
    }

    if (originalEnv.MAYROS_CONFIG_PATH !== undefined) {
      process.env.MAYROS_CONFIG_PATH = originalEnv.MAYROS_CONFIG_PATH;
    } else {
      delete process.env.MAYROS_CONFIG_PATH;
    }

    if (originalEnv.MAYROS_GATEWAY_PORT !== undefined) {
      process.env.MAYROS_GATEWAY_PORT = originalEnv.MAYROS_GATEWAY_PORT;
    } else {
      delete process.env.MAYROS_GATEWAY_PORT;
    }

    if (originalEnv.MAYROS_PROFILE !== undefined) {
      process.env.MAYROS_PROFILE = originalEnv.MAYROS_PROFILE;
    } else {
      delete process.env.MAYROS_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        MAYROS_PROFILE: "dev",
        MAYROS_STATE_DIR: "/tmp/mayros-daemon-state",
