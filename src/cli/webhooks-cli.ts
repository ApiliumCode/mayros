...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "apilium.com/us/doc/mayros/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Mayros hooks")
    ...
    .option("--hook-url <url>", "Mayros hook URL")
    .option("--hook-token <token>", "Mayros hook token")
