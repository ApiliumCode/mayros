...
function applySlotSelectionForPlugin(
  config: MayrosConfig,
  pluginId: string,
): { config: MayrosConfig; warnings: string[] } {
...
function enablePluginInConfig(config: MayrosConfig, pluginId: string): MayrosConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "apilium.com/us/doc/mayros/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "apilium.com/us/doc/mayros/plugin");
      lines.push("");
