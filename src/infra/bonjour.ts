...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MAYROS_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Mayros";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Mayros\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.MAYROS_MDNS_HOSTNAME?.trim() ||
    process.env.MAYROS_MDNS_HOSTNAME?.trim() ||
    "mayros";
  const hostname =
    hostnameRaw
