...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __mayrosDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__mayrosDiagnosticEventsState) {
    globalStore.__mayrosDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__mayrosDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: MayrosConfig): boolean {
