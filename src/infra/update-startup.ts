...
import { resolveMayrosPackageRoot } from "./mayros-root.js";
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveMayrosPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("mayros update")}`,
      );
  ...
  // Best-effort cortex binary version check (non-blocking)
  try {
    const { checkCortexVersion } = await import("../../extensions/shared/cortex-update-check.js");
    const cortex = await checkCortexVersion();
    if (cortex.needsUpdate) {
      const installed = cortex.installedVersion ?? "not installed";
      params.log.info(
        `cortex binary outdated (${installed} < ${cortex.requiredVersion}). Run: ${formatCliCommand("mayros update")}`,
      );
    }
  } catch {
    // Cortex not installed or check failed — silently skip
  }
}
