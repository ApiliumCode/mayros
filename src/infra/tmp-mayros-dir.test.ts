...
import { POSIX_MAYROS_TMP_DIR, resolvePreferredMayrosTmpDir } from "./tmp-mayros-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredMayrosTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `mayros-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredMayrosTmpDir({
    accessSync,
...
describe("resolvePreferredMayrosTmpDir", () => {
  it("prefers /tmp/mayros when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_MAYROS_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/mayros when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MAYROS_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/mayros when /tmp/mayros is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/mayros when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/mayros is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/mayros is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/mayros is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
