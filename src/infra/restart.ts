...
export function triggerMayrosRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.MAYROS_SYSTEMD_UNIT,
        process.env.MAYROS_PROFILE,
      );
  ...
  const label =
    process.env.MAYROS_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.MAYROS_PROFILE);
