...
import { DEFAULT_HEARTBEAT_FILENAME, LEGACY_HEARTBEAT_FILENAME } from "../agents/workspace.js";
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: MayrosConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: MayrosConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: MayrosConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: MayrosConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: MayrosConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: MayrosConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: MayrosConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
async function resolveHeartbeatPreflight(params: {
  ...
  // Try HOPE.md first, then fall back to legacy HEARTBEAT.md
  const heartbeatFilePath = path.join(workspaceDir, DEFAULT_HEARTBEAT_FILENAME);
  const legacyHeartbeatFilePath = path.join(workspaceDir, LEGACY_HEARTBEAT_FILENAME);
  ...
  try {
    ...
    if (hasErrnoCode(err, "ENOENT")) {
      // Try legacy filename
      try {
        heartbeatFileContent = await fs.readFile(legacyHeartbeatFilePath, "utf-8");
      } catch (legacyErr: unknown) {
        if (hasErrnoCode(legacyErr, "ENOENT")) {
          // Neither file exists — intentional in some setups
          return basePreflight;
        }
        readError = legacyErr;
      }
  ...
  // For non-ENOENT read errors, proceed with heartbeat as before.
  void readError;
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: MayrosConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
