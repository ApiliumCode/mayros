...
function candidateBinDirs(opts: EnsureMayrosPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `mayros` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "mayros");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MAYROS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "mayros"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `mayros` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMayrosCliOnPath(opts: EnsureMayrosPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MAYROS_PATH_BOOTSTRAPPED)) {
    return;
  }
