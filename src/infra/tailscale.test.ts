...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.MAYROS_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.MAYROS_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.MAYROS_TEST_TAILSCALE_BINARY;
    } else {
      process.env.MAYROS_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
