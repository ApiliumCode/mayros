...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MayrosConfig | null = null;
  ...
  get(key: string, cfg: MayrosConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MayrosConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: MayrosConfig): void {
