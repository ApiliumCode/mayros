...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__mayros_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "mayros-root");
...
describe("resolveMayrosPackageRoot", () => {
  let resolveMayrosPackageRoot: typeof import("./mayros-root.js").resolveMayrosPackageRoot;
  let resolveMayrosPackageRootSync: typeof import("./mayros-root.js").resolveMayrosPackageRootSync;

  beforeAll(async () => {
    ({ resolveMayrosPackageRoot, resolveMayrosPackageRootSync } = await import("./mayros-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "mayros");
    const pkgRoot = path.join(project, "node_modules", "mayros");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "mayros" }));

    expect(resolveMayrosPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "mayros");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "mayros.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "mayros" }));

    expect(resolveMayrosPackageRootSync({ argv1: bin })).toBe(realPkg);
  });

  it("falls back when argv1 realpath throws", async () => {
    const { resolveMayrosPackageRootSync } = await import("./mayros-root.js");
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveMayrosPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-mayros package roots", async () => {
    const pkgRoot = fx("not-mayros");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-mayros" }));

    expect(resolveMayrosPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveMayrosPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    const { resolveMayrosPackageRoot } = await import("./mayros-root.js");

    await expect(resolveMayrosPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
