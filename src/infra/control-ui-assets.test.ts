...
vi.mock("./mayros-root.js", () => ({
  resolveMayrosPackageRoot: vi.fn(async () => null),
  resolveMayrosPackageRootSync: vi.fn(() => null),
}));
...
let mayrosRoot: typeof import("./mayros-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    mayrosRoot = await import("./mayros-root.js");
  });
  ...
  it("uses resolveMayrosPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/mayros");
    (
      mayrosRoot.resolveMayrosPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/mayros"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "mayros" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "mayros.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-mayros");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/mayros-bundle");
    (
      mayrosRoot.resolveMayrosPackageRootSync as unknown as ReturnType<typeof vi.fn>
