...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.MAYROS_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.MAYROS_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set MAYROS_APNS_TEAM_ID and MAYROS_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.MAYROS_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.MAYROS_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.MAYROS_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set MAYROS_APNS_PRIVATE_KEY_P8 or MAYROS_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading MAYROS_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createMayrosPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    mayros: createMayrosPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
