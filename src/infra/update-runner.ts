...
const DEFAULT_PACKAGE_NAME = "mayros";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "@apilium/mayros"]);
...
function normalizeTag(tag?: string) {
  ...
  if (trimmed.startsWith("mayros@")) {
    return trimmed.slice("mayros@".length);
  }
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "mayros.mjs");
    ...
    const doctorStep = await runStep(
      step("mayros doctor", doctorArgv, gitRoot, { MAYROS_UPDATE_IN_PROGRESS: "1" }),
    );
    ...
    // Auto-update cortex binary if MAYROS update succeeded
    if (!failedStep) {
      await maybeCortexUpdate(steps, progress, stepIndex, gitTotalSteps);
    }
  ...
  if (globalManager) {
    ...
    if (updateStep.exitCode === 0) {
      await maybeCortexUpdate(steps, progress, 1, 2);
    }
...
// ---------------------------------------------------------------------------
// Cortex binary auto-update (best-effort, never fails the overall update)
// ---------------------------------------------------------------------------

async function maybeCortexUpdate(
  steps: UpdateStepResult[],
  ...
  try {
    const { checkCortexVersion, installOrUpdateCortex } =
      await import("../../extensions/shared/cortex-update-check.js");
    const status = await checkCortexVersion();
    if (!status.needsUpdate) {
      return;
    }

    const stepInfo: UpdateStepInfo = {
      name: "cortex binary update",
      command: "installOrUpdateCortex()",
      index: stepIndex,
      total: totalSteps + 1,
    };
    progress?.onStepStart?.(stepInfo);
    ...
    progress?.onStepComplete?.({ ...stepInfo, durationMs, exitCode, stderrTail });
    steps.push({
      name: "cortex binary update",
      command: "installOrUpdateCortex()",
      cwd: "",
    ...
    // Cortex update is best-effort; never fail the main update
  }
}
