...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("mayros-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-mayros-archive.tgz");
  ...
  it("rejects unsupported archive extensions", async () => {
    const dir = await createTempDir("mayros-install-source-utils-");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    const cwd = await createTempDir("mayros-install-source-utils-");
    runCommandWithTimeoutMock.mockResolvedValue({
      stdout: JSON.stringify([
        {
          id: "mayros-plugin@1.2.3",
          name: "mayros-plugin",
          version: "1.2.3",
          filename: "mayros-plugin-1.2.3.tgz",
    ...
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "mayros-plugin-1.2.3.tgz"),
      metadata: {
        name: "mayros-plugin",
        version: "1.2.3",
        resolvedSpec: "mayros-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "mayros-plugin@1.2.3", "--ignore-scripts", "--json"],
