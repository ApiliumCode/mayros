...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "MAYROS_STATE_DIR");
    prevStateDir = process.env.MAYROS_STATE_DIR;
    process.env.MAYROS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveMayrosPackageRoot } = await import("./mayros-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveMayrosPackageRoot).mockResolvedValue("/opt/mayros");
