...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MAYROS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*mayros-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "mayros";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "mayros");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "mayros-gw");
  ...
  records.push(`_mayros-gw._tcp IN PTR ${instanceLabel}._mayros-gw._tcp`);
  records.push(`${instanceLabel}._mayros-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._mayros-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; mayros-content-hash: ${contentHash}\n${contentBody}`;
}
