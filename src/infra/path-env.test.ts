...
let ensureMayrosCliOnPath: typeof import("./path-env.js").ensureMayrosCliOnPath;

describe("ensureMayrosCliOnPath", () => {
  const envKeys = [
    "PATH",
    "MAYROS_PATH_BOOTSTRAPPED",
    "MAYROS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureMayrosCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling mayros exists", () => {
    const tmp = abs("/tmp/mayros-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "mayros");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.MAYROS_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.MAYROS_PATH_BOOTSTRAPPED = "1";
    ensureMayrosCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/mayros-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "mayros");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/mayros-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "mayros");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "mayros");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/mayros-path/case-linuxbrew");
