...
const createCaseDir = async (prefix: string, { skipHeartbeatFile = false } = {}) => {
  ...
  if (!skipHeartbeatFile) {
    await fs.writeFile(path.join(dir, "HOPE.md"), "- Check status\n", "utf-8");
  }
  return dir;
};
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-heartbeat-suite-"));
});
...
describe("runHeartbeatOnce", () => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("mayros-hb");
  ...
  it("skips heartbeat when HEARTBEAT.md is effectively empty (saves API calls)", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(workspaceDir, "HOPE.md"),
        "# HEARTBEAT.md\n\n## Tasks\n\n",
        "utf-8",
      );
  ...
  it("runs heartbeat when HEARTBEAT.md read fails with a non-ENOENT error", async () => {
    ...
    try {
      ...
      await fs.mkdir(path.join(workspaceDir, "HOPE.md"), { recursive: true });
