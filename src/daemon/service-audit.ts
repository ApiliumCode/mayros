...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.MAYROS_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service MAYROS_GATEWAY_TOKEN does not match gateway.auth.token in mayros.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `mayros gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
