...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.MAYROS_GATEWAY_PORT).toBe("18789");
    expect(env.MAYROS_GATEWAY_TOKEN).toBe("secret");
    expect(env.MAYROS_SERVICE_MARKER).toBe("mayros");
    expect(env.MAYROS_SERVICE_KIND).toBe("gateway");
    expect(typeof env.MAYROS_SERVICE_VERSION).toBe("string");
    expect(env.MAYROS_SYSTEMD_UNIT).toBe("mayros-gateway.service");
    if (process.platform === "darwin") {
      expect(env.MAYROS_LAUNCHD_LABEL).toBe("ai.mayros.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", MAYROS_PROFILE: "work" },
      port: 18789,
    });
    expect(env.MAYROS_SYSTEMD_UNIT).toBe("mayros-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.MAYROS_LAUNCHD_LABEL).toBe("ai.mayros.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mayros"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", MAYROS_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mayros-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", MAYROS_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mayros"));
  });

  it("uses MAYROS_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", MAYROS_STATE_DIR: "/var/lib/mayros" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/mayros"));
  });

  it("expands ~ in MAYROS_STATE_DIR", () => {
    const env = { HOME: "/Users/test", MAYROS_STATE_DIR: "~/mayros-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/mayros-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { MAYROS_STATE_DIR: "C:\\State\\mayros" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\mayros");
  });
});
