...
function createMemorySearchCfg(options: { workspaceDir: string; indexPath: string }): MayrosConfig {
  return {
    agents: {
      defaults: {
...
describe("MemoryIndexManager.readFile", () => {
  ...
  beforeEach(async () => {
    resetEmbeddingMocks();
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-mem-read-"));
