...
export function isCommandEnabled(cfg: MayrosConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: MayrosConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as MayrosConfig),
...
export function getCommandDetection(_cfg?: MayrosConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: MayrosConfig) {
