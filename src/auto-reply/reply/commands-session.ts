...
import { scheduleGatewaySigusr1Restart, triggerMayrosRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Mayros in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMayrosRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Mayros via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
