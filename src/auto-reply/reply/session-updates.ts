...
export async function prependSystemEvents(params: {
  ...
  const compactSystemEvent = (line: string): string | null => {
    ...
    // The heartbeat prompt starts with "Read HOPE.md" - cron payloads won't match this
  ...
  const resolveSystemEventTimezone = (cfg: MayrosConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: MayrosConfig) => {
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.MAYROS_TEST_FAST === "1") {
