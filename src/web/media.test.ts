...
import { resolvePreferredMayrosTmpDir } from "../infra/tmp-mayros-dir.js";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredMayrosTmpDir(), "mayros-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub MAYROS_STATE_DIR.
    // Also keep it outside the Mayros temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["MAYROS_STATE_DIR"]);
    process.env.MAYROS_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "mayros-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredMayrosTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Mayros state workspace and sandbox roots", async () => {
  ...
  it("rejects default Mayros state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

    await expect(
      loadWebMedia(path.join(stateDir, "workspace-atlas", "tmp", "render.bin"), {
        maxBytes: 1024 * 1024,
        readFile,
      }),
    ).rejects.toMatchObject({ code: "path-not-allowed" });
  ...
  it("allows per-agent workspace-* paths with explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
    const agentWorkspaceDir = path.join(stateDir, "workspace-atlas");
