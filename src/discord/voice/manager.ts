...
function resolveVoiceTtsConfig(params: { cfg: MayrosConfig; override?: TtsConfig }): {
  cfg: MayrosConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredMayrosTmpDir(), "discord-voice-"));
