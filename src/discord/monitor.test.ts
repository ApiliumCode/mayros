...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Mayros")).toBe("friends-of-mayros");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "testuser", "Friends of Mayros"],
    ...
    expect(allowListMatches(allow, { name: "testuser" })).toBe(true);
    expect(allowListMatches(allow, { name: "friends-of-mayros" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-mayros" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Mayros"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-mayros");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-mayros": { slug: "friends-of-mayros" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Mayros"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["mayros-dm"],
        channelId: "1",
        channelName: "Mayros DM",
        channelSlug: "mayros-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
