...
describe("config io write", () => {
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".mayros", "mayros.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".mayros", "logs", "config-audit.jsonl");
  ...
  it("persists caller changes onto resolved config without leaking runtime defaults", async () => {
    await withTempHome("mayros-config-io-", async (home) => {
  ...
  it("keeps env refs in arrays when appending entries", async () => {
    await withTempHome("mayros-config-io-", async (home) => {
      const configPath = path.join(home, ".mayros", "mayros.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        JSON.stringify(
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withTempHome("mayros-config-io-", async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        env: {
          MAYROS_WATCH_MODE: "1",
          MAYROS_WATCH_SESSION: "watch-session-1",
          MAYROS_WATCH_COMMAND: "gateway --force",
