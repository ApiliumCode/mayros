...
const ETC_MAYROS_DIR = path.join(ROOT_DIR, "etc", "mayros");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "mayros.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcMayrosPath(...parts: string[]) {
  return path.join(ETC_MAYROS_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcMayrosPath("agents.json");
  ...
  it("rejects parent directory traversal escaping config directory (CWE-22)", () => {
    ...
    expect(() => resolve(obj, files, configPath("sub", "mayros.json"))).toThrow(ConfigIncludeError);
    expect(() => resolve(obj, files, configPath("sub", "mayros.json"))).toThrow(
    ...
    itSymlink("allows include files when the config root path is a symlink", async () => {
      ...
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "mayros.json"),
        );
