...
import type { MayrosConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareMayrosVersions } from "./version.js";
...
function coerceConfig(value: unknown): MayrosConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as MayrosConfig;
}
...
function stampConfigVersion(cfg: MayrosConfig): MayrosConfig {
...
function warnIfConfigFromFuture(cfg: MayrosConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareMayrosVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Mayros (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as MayrosConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): MayrosConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as MayrosConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: MayrosConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as MayrosConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MAYROS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.MAYROS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.MAYROS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.MAYROS_WATCH_SESSION === "string" &&
        deps.env.MAYROS_WATCH_SESSION.trim().length > 0
          ? deps.env.MAYROS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.MAYROS_WATCH_COMMAND === "string" &&
        deps.env.MAYROS_WATCH_COMMAND.trim().length > 0
          ? deps.env.MAYROS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `MAYROS_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MAYROS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MAYROS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): MayrosConfig {
