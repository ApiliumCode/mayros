...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("mayros-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "mayros-sandbox-browser-global" },
      agentBrowser: { network: "mayros-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("mayros-sandbox-browser-agent");
  });
