...
import type { ConfigFileSnapshot } from "./types.mayros.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.mayros/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/mayros/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/mayros/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/mayros/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/mayros/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = MayrosSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "MayrosConfig";
    const hints = mapSensitivePaths(MayrosSchema, "", {});
