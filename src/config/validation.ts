...
import type { MayrosConfig, ConfigValidationIssue } from "./types.js";
import { MayrosSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: MayrosConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: MayrosConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MayrosSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as MayrosConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as MayrosConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as MayrosConfig,
  };
}
