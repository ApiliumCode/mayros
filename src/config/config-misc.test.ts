...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = MayrosSchema.safeParse({
      $schema: "https://apilium.com/us/products/maryos/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://apilium.com/us/products/maryos/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = MayrosSchema.safeParse({ $schema: 123 });
...
describe("ui.seamColor", () => {
  ...
  it("rejects non-hex colors", () => {
    const res = validateConfigObject({ ui: { seamColor: "atlas" } });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = MayrosSchema.safeParse({
      cron: {
        enabled: true,
