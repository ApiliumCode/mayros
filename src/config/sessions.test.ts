...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-mayros",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-mayros#general");
  });
  ...
  it("derives session transcripts dir from MAYROS_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MAYROS_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    const prev = process.env.MAYROS_STATE_DIR;
    process.env.MAYROS_STATE_DIR = "/custom/state";
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.MAYROS_STATE_DIR = prev;
      }
    }
  });
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const prev = process.env.MAYROS_STATE_DIR;
    const stateDir = path.resolve("/home/user/.mayros");
  ...
  it("resolves cross-agent paths when MAYROS_STATE_DIR differs from stored paths", () => {
    const prev = process.env.MAYROS_STATE_DIR;
    process.env.MAYROS_STATE_DIR = path.resolve("/different/state");
    try {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    const prev = process.env.MAYROS_STATE_DIR;
    process.env.MAYROS_STATE_DIR = path.resolve("/home/user/.mayros");
    try {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
