...
function telegramCfg(): MayrosConfig {
  return { channels: { telegram: { botToken: "tok" } } } as MayrosConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as MayrosConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as MayrosConfig;
