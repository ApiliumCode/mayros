...
import { discoverMayrosPlugins } from "../../plugins/discovery.js";
import type { MayrosPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, MayrosPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["MAYROS_PLUGIN_CATALOG_PATHS", "MAYROS_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<MayrosPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: MayrosPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: MayrosPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverMayrosPlugins({ workspaceDir: options.workspaceDir });
