...
function cfg(accounts?: Record<string, unknown> | null): MayrosConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as MayrosConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as MayrosConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as MayrosConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as MayrosConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as MayrosConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as MayrosConfig)).toBe("default");
    });
  });
});
