...
function setTelegramDmPolicy(cfg: MayrosConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
async function noteTelegramTokenHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Website: https://apilium.com/us/products/mayros",
    ].join("\n"),
    "Telegram bot token",
  );
}
...
async function noteTelegramUserIdHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("mayros logs --follow")}\` (safest)`,
