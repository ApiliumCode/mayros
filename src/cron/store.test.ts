...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MAYROS_HOME for tilde expansion", () => {
    vi.stubEnv("MAYROS_HOME", "/srv/mayros-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/mayros-home", "cron", "jobs.json"));
  });
});
