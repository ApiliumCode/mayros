...
function summarizeGroupPolicy(cfg: MayrosConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: MayrosConfig): boolean {
...
function collectModels(cfg: MayrosConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: MayrosConfig): Set<string> {
  const baseCfg: MayrosConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: MayrosConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: MayrosConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: MayrosConfig): boolean {
...
function isBrowserEnabled(cfg: MayrosConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: MayrosConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: MayrosConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep MAYROS_STATE_DIR on a local-only volume and re-run "${formatCliCommand("mayros security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: MayrosConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer MAYROS_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const mayrosGatewayToken =
    typeof env.MAYROS_GATEWAY_TOKEN === "string" && env.MAYROS_GATEWAY_TOKEN.trim()
      ? env.MAYROS_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-mayros-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: MayrosConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: MayrosConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: mayros-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: MayrosConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: MayrosConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: MayrosConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: MayrosConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
