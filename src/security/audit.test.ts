...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: MayrosConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withStateDir = async (label: string, fn: (tmp: string) => Promise<void>) => {
    const prevStateDir = process.env.MAYROS_STATE_DIR;
    const tmp = await makeTmpDir(label);
    process.env.MAYROS_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      await fn(tmp);
    } finally {
      if (prevStateDir == null) {
        delete process.env.MAYROS_STATE_DIR;
      } else {
        process.env.MAYROS_STATE_DIR = prevStateDir;
      }
    }
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.MAYROS_GATEWAY_TOKEN;
    const prevPassword = process.env.MAYROS_GATEWAY_PASSWORD;
    delete process.env.MAYROS_GATEWAY_TOKEN;
    delete process.env.MAYROS_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.MAYROS_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.MAYROS_GATEWAY_PASSWORD;
      } else {
        process.env.MAYROS_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "mayros.json");
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("mayros-sbx-browser-old\nmayros-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "mayros-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "mayros-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("mayros-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("mayros-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "mayros-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "mayros-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-mayros.json");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: MayrosConfig = { session: { dmScope: "main" } };
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: MayrosConfig = { gateway: { mode: "local" } };
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.MAYROS_GATEWAY_TOKEN;
    process.env.MAYROS_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: MayrosConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.mayros",
      configPath: "/Users/test/Dropbox/.mayros/mayros.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: MayrosConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "mayros.json"),
      });
    ...
    const cfg: MayrosConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@apilium/mayros-voice-call",
          },
        },
      },
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@apilium/mayros-test-hooks",
            },
          },
        },
      },
    ...
    const cfg: MayrosConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@apilium/mayros-voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@apilium/mayros-test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@apilium/mayros-voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@apilium/mayros-test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        mayros: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.MAYROS_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.MAYROS_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.MAYROS_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.MAYROS_GATEWAY_PASSWORD;
      } else {
        process.env.MAYROS_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.MAYROS_GATEWAY_TOKEN = "env-token";
      const { probeGatewayFn, getAuth } = makeProbeCapture();
      const cfg: MayrosConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.MAYROS_GATEWAY_PASSWORD = "env-pass";
      const { probeGatewayFn, getAuth } = makeProbeCapture();
      const cfg: MayrosConfig = {
        gateway: {
          mode: "remote",
