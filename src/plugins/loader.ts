...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: MayrosPluginDefinition;
  register?: MayrosPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as MayrosPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as MayrosPluginDefinition;
...
export function loadMayrosPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "mayros/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "mayros/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: MayrosPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as MayrosPluginModule;
    } catch (err) {
