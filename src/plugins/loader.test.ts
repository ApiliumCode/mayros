...
import { loadMayrosPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `mayros-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.MAYROS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "mayros.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.MAYROS_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadMayrosPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.MAYROS_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.MAYROS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadMayrosPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadMayrosPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadMayrosPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.MAYROS_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ MAYROS_STATE_DIR: stateDir }, () => {
