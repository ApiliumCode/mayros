...
const loadMayrosPluginsMock = vi.fn();

vi.mock("./loader.js", () => ({
  loadMayrosPlugins: (params: unknown) => loadMayrosPluginsMock(params),
}));
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadMayrosPluginsMock.mockReturnValue(registry);
  return registry;
}

describe("resolvePluginTools optional tools", () => {
  beforeEach(() => {
    loadMayrosPluginsMock.mockReset();
  });
