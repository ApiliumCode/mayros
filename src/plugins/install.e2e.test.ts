...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `mayros-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        mayros: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      mayros: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.mayros/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without mayros.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@apilium/mayros-nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("mayros.extensions");
  });
...
describe("installPluginFromDir", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@apilium/mayros-test-plugin",
        version: "0.0.1",
        mayros: { extensions: ["./dist/index.js"] },
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@apilium/mayros-voice-call@0.0.1",
              name: "@apilium/mayros-voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@apilium/mayros-voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@apilium/mayros-voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@apilium/mayros-voice-call@0.0.1",
    });
