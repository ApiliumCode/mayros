...
function createSinglePluginWithEmptySlotsConfig(): MayrosConfig {
  return {
    plugins: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "mayros-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-mayros-extensions/my-plugin",
      },
      extensionsDir,
    });
