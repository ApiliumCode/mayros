...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".mayros");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.MAYROS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$MAYROS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via MAYROS_STATE_DIR.
