...
function listAgents(cfg: MayrosConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: MayrosConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: MayrosConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<MayrosConfig, EvaluatedBindingsCache>();
