...
describe("gateway auth", () => {
  it("resolves token/password from MAYROS gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          MAYROS_GATEWAY_TOKEN: "env-token",
          MAYROS_GATEWAY_PASSWORD: "env-password",
  ...
  it("ignores unrelated env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          UNRELATED_TOKEN: "not-a-gateway-token",
          UNRELATED_PASSWORD: "not-a-gateway-password",
        } as NodeJS.ProcessEnv,
      }),
    ).toMatchObject({
      mode: "token",
