...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "MAYROS_STATE_DIR",
  "MAYROS_CONFIG_PATH",
  "MAYROS_SKIP_BROWSER_CONTROL_SERVER",
  "MAYROS_SKIP_GMAIL_WATCHER",
  "MAYROS_SKIP_CANVAS_HOST",
  "MAYROS_BUNDLED_PLUGINS_DIR",
  "MAYROS_SKIP_CHANNELS",
  "MAYROS_SKIP_PROVIDERS",
  "MAYROS_SKIP_CRON",
  "MAYROS_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mayros-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MAYROS_STATE_DIR = path.join(tempHome, ".mayros");
...
function applyGatewaySkipEnv() {
  ...
  process.env.MAYROS_SKIP_PROVIDERS = "1";
  process.env.MAYROS_SKIP_CRON = "1";
  process.env.MAYROS_TEST_MINIMAL_GATEWAY = "1";
  process.env.MAYROS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "mayros-test-no-bundled-extensions")
    : "mayros-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "mayros-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".mayros-test");
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["MAYROS_GATEWAY_TOKEN"]);
  const prev = process.env.MAYROS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MAYROS_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.MAYROS_GATEWAY_PASSWORD;
