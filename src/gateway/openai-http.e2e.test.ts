    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "mayros", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-mayros-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-mayros-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "mayros", messages: [{ role: "user", content: "hi" }] },
          {
            "x-mayros-agent-id": "beta",
            "x-mayros-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
