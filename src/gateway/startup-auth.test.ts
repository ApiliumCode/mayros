...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: MayrosConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: MayrosConfig) {
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          MAYROS_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
