...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.MAYROS_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.MAYROS_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.MAYROS_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.MAYROS_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
    ...
    test("connect (req) handshake prefers MAYROS_VERSION over service version", async () => {
      await withRuntimeVersionEnv(
        {
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `mayros-test-device-${randomUUID()}.json`),
      });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.MAYROS_GATEWAY_TOKEN;
      process.env.MAYROS_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { getPairedDevice } = await import("../infra/device-pairing.js");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "mayros-device-scope-"));
