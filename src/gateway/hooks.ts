...
export function resolveHooksConfig(cfg: MayrosConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: MayrosConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-mayros-token"] === "string" ? req.headers["x-mayros-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
