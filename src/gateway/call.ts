...
export function buildGatewayConnectionDetails(
  options: { config?: MayrosConfig; url?: string; configPath?: string } = {},
...
function resolveGatewayCredentials(context: ResolvedGatewayCallContext): {
  ...
  const token =
    ...
    (!context.urlOverride
      ? context.isRemoteMode
        ...
        : trimToUndefined(process.env.MAYROS_GATEWAY_TOKEN) ||
          trimToUndefined(process.env.MAYROS_GATEWAY_TOKEN) ||
  ...
  const password =
    ...
    (!context.urlOverride
      ? trimToUndefined(process.env.MAYROS_GATEWAY_PASSWORD) ||
        trimToUndefined(process.env.MAYROS_GATEWAY_PASSWORD) ||
        (context.isRemoteMode
