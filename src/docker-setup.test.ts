...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "mayros-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    MAYROS_GATEWAY_TOKEN: "test-token",
    MAYROS_CONFIG_DIR: join(sandbox.rootDir, "config"),
    MAYROS_WORKSPACE_DIR: join(sandbox.rootDir, "mayros"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        MAYROS_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        MAYROS_EXTRA_MOUNTS: undefined,
        MAYROS_HOME_VOLUME: "mayros-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MAYROS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("MAYROS_EXTRA_MOUNTS=");
    expect(envFile).toContain("MAYROS_HOME_VOLUME=mayros-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("mayros-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("mayros-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg MAYROS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });

  it("rejects injected multiline MAYROS_EXTRA_MOUNTS values", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    expect(result.stderr).toContain("MAYROS_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid MAYROS_EXTRA_MOUNTS mount format", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        MAYROS_EXTRA_MOUNTS: "bad mount spec",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
  ...
  it("rejects invalid MAYROS_HOME_VOLUME names", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("MAYROS_HOME_VOLUME must match");
  });
